...
evaluation:
  metrics:
    - name: parse_latency_ms
      command: "bun test packages/opencode/src/agent-script/parser.test.ts"
      threshold: 5000
    - name: preview_summary_complete
      command: "bun run --filter @opencode-ai/opencode typecheck"
      threshold: 1
  compare: baseline
...
unsupported_feature_policy:
  required: [interop, runtime, capability_negotiation, memory_kernel, evidence_graph, trust, requirements, evaluation, release, roles, channels, imports, reasoning_privacy]
