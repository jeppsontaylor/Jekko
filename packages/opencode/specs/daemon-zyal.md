...
`ZYAL` is the host-enforced daemon/runbook format for OpenCode.
...
ZYAL v1 supports an optional top-level `incubator` block for hard tasks. The incubator is host-enforced: each task gets finite typed passes, durable SQLite-backed artifacts, generated mirrors under `.opencode/daemon/<runID>/tasks/<taskID>/`, and a runtime-owned promotion gate.
