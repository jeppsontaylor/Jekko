...
Replace runtime Bun shell template-tag usage in `packages/jekko/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/jekko/src` is removed except:
