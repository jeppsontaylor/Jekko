...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "jekko.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "jekko.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "jekko.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "jekko.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["jekko.json", "jekko.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "jekko.json"),
        path.join(tmp.path, "jekko.jsonc"),
        path.join(project, "jekko.json"),
        path.join(project, "jekko.jsonc"),
      ])
    })
  })
