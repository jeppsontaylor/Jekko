...
import {
  ...
  timeout,
  waitRequest,
  waitStreamingRequest,
} from "./llm.stream.shared"
...
describe("session.llm.stream", () => {
  ...
  test("adds jnoccio identity headers for jekko providers", async () => {
    const server = state.server
    if (!server) {
      throw new Error("Server not initialized")
    }

    const providerID = "jekko"
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        const resolved = await getModel(ProviderID.make(providerID), ModelID.make(model.id))
        const sessionID = SessionID.make("session-jekko-headers")
        const agent = {
          name: "test",
          mode: "primary",
          options: {},
        ...
        const user = {
          id: MessageID.make("user-jekko-headers"),
          sessionID,
          role: "user",
          time: { created: Date.now() },
          agent: agent.name,
        ...
        expect(capture.headers.get("x-jekko-session")).toBe(sessionID)
        expect(capture.headers.get("x-jekko-request")).toBe(user.id)
  ...
  test("adds daemon identity headers for non-jekko providers without jekko request attribution", async () => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        const resolved = await getModel(ProviderID.make(providerID), ModelID.make(model.id))
        const sessionID = SessionID.make("session-non-jekko-headers")
        const agent = {
          name: "test",
          mode: "primary",
          options: {},
        ...
        const user = {
          id: MessageID.make("user-non-jekko-headers"),
          sessionID,
          role: "user",
          time: { created: Date.now() },
          agent: agent.name,
  ...
  test("service stream cancellation cancels provider response body promptly", async () => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        await Promise.race([pending.requestAborted, timeout(10000)])
