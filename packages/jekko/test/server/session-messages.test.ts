...
async function withoutWatcher<T>(fn: () => Promise<T>) {
  ...
  const prev = process.env.JEKKO_EXPERIMENTAL_DISABLE_FILEWATCHER
  process.env.JEKKO_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
  try {
    return await fn()
  } finally {
    if (prev === undefined) delete process.env.JEKKO_EXPERIMENTAL_DISABLE_FILEWATCHER
    else process.env.JEKKO_EXPERIMENTAL_DISABLE_FILEWATCHER = prev
  }
}
