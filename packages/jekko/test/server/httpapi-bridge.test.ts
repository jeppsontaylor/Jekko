...
function app(input?: { password?: string; username?: string }) {
  ...
  Flag.JEKKO_SERVER_PASSWORD = input?.password
  Flag.JEKKO_SERVER_USERNAME = input?.username
...
describe("HttpApi server", () => {
  ...
  test("requires credentials when auth is enabled", async () => {
    ...
    const [missing, bad, good] = await Promise.all([
      ...
      app({ password: "secret" }).request(fileUrl(), {
        headers: {
          authorization: authorization("jekko", "wrong"),
          "x-jekko-directory": tmp.path,
        },
      }),
      app({ password: "secret" }).request(fileUrl(), {
        headers: {
          authorization: authorization("jekko", "secret"),
          "x-jekko-directory": tmp.path,
        },
      }),
    ])
  ...
  test("accepts auth_token query credentials", async () => {
    ...
    const response = await app({ password: "secret" }).request(
      fileUrl({ token: Buffer.from("jekko:secret").toString("base64") }),
      {
        headers: {
          "x-jekko-directory": tmp.path,
        },
  ...
  test("serves global health from Effect HttpApi", async () => {
    const response = await app().request(`${GlobalPaths.health}?directory=/does/not/exist/jekko-test`)
