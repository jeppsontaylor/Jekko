...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under jekko.", () => {
    expect(paramToAttributeKey("name")).toBe("jekko.name")
    expect(paramToAttributeKey("slug")).toBe("jekko.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["jekko.sessionID"]).toBeUndefined()
    expect(attrs["jekko.messageID"]).toBeUndefined()
    expect(attrs["jekko.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under jekko. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["jekko.name"]).toBe("exa")
