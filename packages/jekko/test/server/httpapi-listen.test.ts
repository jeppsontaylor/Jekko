...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.JEKKO_SERVER_PASSWORD
  else process.env.JEKKO_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.JEKKO_SERVER_USERNAME
  else process.env.JEKKO_SERVER_USERNAME = original.envUsername
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-jekko-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-jekko-ticket": "1" }),
  ...
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-jekko-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  testPty("PTY connect token requires matching directory across mint and connect", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-jekko-ticket": "1" },
      })
