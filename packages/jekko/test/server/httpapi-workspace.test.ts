...
function request(path: string, directory: string, init: RequestInit = {}, httpApi = true) {
  return Effect.promise(() => {
    Flag.JEKKO_EXPERIMENTAL_HTTPAPI = httpApi
    const headers = new Headers(init.headers)
    headers.set("x-jekko-directory", directory)
    ...
    Effect.gen(function* () {
      ...
      try {
        const response = yield* request(url.toString(), dir, {
          ...
          body: JSON.stringify({ $schema: "https://jekko.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-jekko-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-jekko-workspace")
      } finally {
