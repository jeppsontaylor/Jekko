...
const syncResponse = (request: HttpServerRequest.HttpServerRequest) => {
  ...
  // jankurai:allow HLT-001-DEAD-MARKER reason=test-unmatched-router-return expires=2027-01-01
  return undefined
}
...
describe("HttpApi workspace routing middleware", () => {
  it.live("proxies remote workspace HTTP requests through the selected workspace target", () =>
    Effect.gen(function* () {
      ...
      // This starts a second HTTP server that stands in for the jekko server
      ...
      const response = yield* HttpClientRequest.patch(`/probe?workspace=${workspace.id}&keep=yes`).pipe(
        HttpClientRequest.setHeaders({
          ...
          "x-jekko-directory": "/secret/path",
          "x-jekko-workspace": "internal",
        }),
        HttpClient.execute,
      )
      ...
      expect(forwarded?.headers["x-jekko-directory"]).toBeUndefined()
      expect(forwarded?.headers["x-jekko-workspace"]).toBeUndefined()
    }),
  )
  ...
  it.live("uses directory query/header alternative_path when no workspace is selected", () =>
    Effect.gen(function* () {
      ...
      const headerResponse = yield* HttpClientRequest.get("/probe").pipe(
        HttpClientRequest.setHeader("x-jekko-directory", headerDir),
        HttpClient.execute,
      )
