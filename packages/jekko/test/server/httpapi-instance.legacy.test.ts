...
describe("instance HttpApi", () => {
  test("serves catalog read endpoints through Hono bridge", async () => {
    ...
    const [commands, agents, skills, lsp, formatter] = await Promise.all([
      app().request(InstancePaths.command, { headers: { "x-jekko-directory": tmp.path } }),
      app().request(InstancePaths.agent, { headers: { "x-jekko-directory": tmp.path } }),
      app().request(InstancePaths.skill, { headers: { "x-jekko-directory": tmp.path } }),
      app().request(InstancePaths.lsp, { headers: { "x-jekko-directory": tmp.path } }),
      app().request(InstancePaths.formatter, { headers: { "x-jekko-directory": tmp.path } }),
    ])
  ...
  test("serves project git init through Hono bridge", async () => {
    ...
    const response = await app().request("/project/git/init", {
      method: "POST",
      headers: { "x-jekko-directory": tmp.path },
    })
    ...
    const current = await app().request("/project/current", { headers: { "x-jekko-directory": tmp.path } })
  ...
  test("serves project update through Hono bridge", async () => {
    ...
    const list = await app().request("/project", { headers: { "x-jekko-directory": tmp.path } })
