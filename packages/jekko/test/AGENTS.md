...
- `config?: Partial<Config.Info>` - Write an `jekko.json` config file
...
- Directories are created in the system temp folder with prefix `jekko-test-`
...
it.instance("uses the temp directory", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("jekko-test-")
  }),
)
```
