...
test("migrates historical tools config to permissions - deny", async () => {
...
test("migrates historical write tool to edit permission", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, "jekko.json"),
...
// Note: preload.ts sets JEKKO_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("migrates historical edit tool to edit permission", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, "jekko.json"),
