...
afterEach(async () => {
  delete process.env.JEKKO_CONFIG
  delete process.env.JEKKO_TUI_CONFIG
  await fs.rm(path.join(Global.Path.config, "jekko.json"), { force: true }).catch(() => {})
  await fs.rm(path.join(Global.Path.config, "jekko.jsonc"), { force: true }).catch(() => {})
...
test("keeps server and tui plugin merge semantics aligned", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const local = path.join(dir, ".jekko")
      await fs.mkdir(local, { recursive: true })

      await Bun.write(
        path.join(Global.Path.config, "jekko.json"),
        JSON.stringify(
          {
...
test("loads tui config with the same precedence order as server config paths", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await fs.mkdir(path.join(dir, ".jekko"), { recursive: true })
      await Bun.write(
        path.join(dir, ".jekko", "tui.json"),
...
test("migrates tui-specific keys from jekko.json when tui.json does not exist", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
  ...
  const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "jekko.json")))
  expect(server.theme).toBeUndefined()
  expect(server.keybinds).toBeUndefined()
  expect(server.tui).toBeUndefined()
  expect(await Filesystem.exists(path.join(tmp.path, "jekko.json.tui-migration.bak"))).toBe(true)
...
test("skips migration when jekko.jsonc is syntactically invalid", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "jekko.jsonc.tui-migration.bak"))).toBe(false)
  const source = await Filesystem.readText(path.join(tmp.path, "jekko.jsonc"))
...
test("skips migration when tui.json already exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "jekko.json"), JSON.stringify({ theme: "historical" }, null, 2))
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "jekko.json.tui-migration.bak"))).toBe(false)
})

test("continues loading tui config when historical source cannot be stripped", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "jekko.json"), JSON.stringify({ theme: "readonly-theme" }, null, 2))
    },
  })

  const source = path.join(tmp.path, "jekko.json")
...
test("migration backup preserves JSONC comments", async () => {
  ...
  const backup = await Filesystem.readText(path.join(tmp.path, "jekko.jsonc.tui-migration.bak"))
...
test("migrates historical tui keys across multiple jekko.json levels", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const nested = path.join(dir, "apps", "client")
      await fs.mkdir(nested, { recursive: true })
      await Bun.write(path.join(dir, "jekko.json"), JSON.stringify({ theme: "root-theme" }, null, 2))
      await Bun.write(path.join(nested, "jekko.json"), JSON.stringify({ theme: "nested-theme" }, null, 2))
    },
  })
...
test("project config takes precedence over JEKKO_TUI_CONFIG (matches JEKKO_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.JEKKO_TUI_CONFIG = custom
    },
  })
...
test("JEKKO_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from JEKKO_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const customDir = path.join(dir, "custom")
      await fs.mkdir(customDir, { recursive: true })
      await Bun.write(path.join(customDir, "jekko.json"), JSON.stringify({ model: "test/model" }))
      await Bun.write(path.join(customDir, "tui.json"), JSON.stringify({ theme: "should-not-load" }))
      process.env.JEKKO_CONFIG = path.join(customDir, "jekko.json")
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.theme).toBeUndefined()
...
test("loads .jekko/tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".jekko"), { recursive: true })
      await Bun.write(path.join(dir, ".jekko", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.diff_style).toBe("stacked")
...
test("silently skips malformed tui.json — load failures degrade to {}", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(dir, ".jekko", "tui.json"), JSON.stringify({ theme: "alternative_path" }))
    },
  })

  const config = await getTuiConfig(tmp.path)
  // Project tui.json is malformed → silently skipped (logs a warning)
  // .jekko/tui.json (lower precedence in this path) still loads
...
test("silently skips non-ENOENT read failures (e.g. tui.json is a directory) — alternative_path layer still loads", async () => {
  ...
  const config = await getTuiConfig(tmp.path)
  // Did NOT crash; .jekko/tui.json (lower precedence) still loads.
  expect(config.theme).toBe("alternative_path")
})
