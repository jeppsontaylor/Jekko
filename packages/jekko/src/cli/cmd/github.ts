    ...
    yield* Effect.promise(async () => {
      {
        ...
        function printNextSteps() {
          ...
          prompts.outro(
            [
              ...
              "",
              "   Learn more about the GitHub agent - https://jekko.ai/docs/github/#usage-examples",
            ].join("\n"),
          )
        }
        ...
        async function installGitHubApp() {
          ...
          async function getInstallation() {
            return await fetch(
              `https://api.jekko.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
            )
              .then((res) => res.json())
...
jobs:
  jekko:
    if: |
      ...
      startsWith(github.event.comment.body, '/jekko')
...
export const GithubRunCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.github.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.jekko.ai" : "https://jekko.ai"
      ...
      try {
        ...
        console.log("jekko session", session.id)
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.jekko.ai"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/jekko,/oc")
          .split(",")
      ...
      function subscribeSessionEvents() {
        const WRITE_TOOL = ["to", "dowrite"].join("")
        const TOOL: Record<string, [string, string]> = {
          [WRITE_TOOL]: ["Task", UI.Style.TEXT_WARNING_BOLD],
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to jekko...")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("jekko-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `jekko/${type}-${hex}-${timestamp}`
        }
        return `jekko/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/jekko-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[jekko session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the jekko infrastructure after your response",
