...
const TUI_SCHEMA_URL = "https://jekko.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from jekko.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const jekko = await jekkoFiles(input)
...
async function jekkoFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "jekko"),
    ...(await Filesystem.findUp(["jekko.json", "jekko.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "jekko"))
  }
  if (Flag.JEKKO_CONFIG) files.push(Flag.JEKKO_CONFIG)
