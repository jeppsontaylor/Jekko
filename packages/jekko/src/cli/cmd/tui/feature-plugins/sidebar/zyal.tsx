...
import { useZyalFlash, useZyalMetrics, formatZyalRuntime, zyalFleetHardCap } from "../../context/zyal-flash"
...
function View(props: { api: TuiPluginApi }) {
  ...
  const workersLine = createMemo(() => {
    ...
    const max = m.workersMax || zyalFleetHardCap()
    return `${active} / ${max}`
  })
  ...
  const jnoccioLine = createMemo(() => {
    ...
    const max = m.jnoccioMaxInstances ?? zyalFleetHardCap()
    return `${inst} / ${max}`
  })
