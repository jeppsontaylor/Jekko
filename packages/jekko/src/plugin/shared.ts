...
export async function checkPluginCompatibility(target: string, jekkoVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(jekkoVersion) || semver.major(jekkoVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.jekko
  if (typeof range !== "string") return
  if (!semver.satisfies(jekkoVersion, range)) {
    throw new Error(`Plugin requires jekko ${range} but running ${jekkoVersion}`)
  }
}
