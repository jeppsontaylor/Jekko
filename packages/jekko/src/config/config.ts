...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "jekko.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "jekko.jsonc")))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (Flag.JEKKO_CONFIG) {
          yield* merge(Flag.JEKKO_CONFIG, yield* loadFile(Flag.JEKKO_CONFIG))
        ...
        if (Flag.JEKKO_CONFIG_DIR) {
          log.debug("loading config from JEKKO_CONFIG_DIR", { path: Flag.JEKKO_CONFIG_DIR })
        }
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under `.jekko/plugin(s)` are already local files, so ConfigPlugin.load
        ...
        if (Flag.JEKKO_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.JEKKO_PERMISSION))
        }
