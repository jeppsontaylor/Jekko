...
const MANAGED_PLIST_DOMAIN = "ai.jekko.managed"

// Keys injected by macOS/MDM into the managed plist that are not Jekko config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/jekko"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "jekko")
    default:
      return "/etc/jekko"
  }
}
...
export function managedConfigDir() {
  return process.env.JEKKO_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
