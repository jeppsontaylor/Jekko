...
function createHono(opts: CorsOptions, selection: ServerBackend.Selection = ServerBackend.force(select(), "hono")) {
  ...
  if (Flag.JEKKO_WORKSPACE_ID) {
    return {
      app: app
        .use(InstanceMiddleware(Flag.JEKKO_WORKSPACE_ID ? WorkspaceID.make(Flag.JEKKO_WORKSPACE_ID) : undefined))
        .use(FenceMiddleware)
 ...
 * `PublicApi` is `JekkoHttpApi` annotated with the `matchCompatOpenApi`
...
async function listenHttpApi(opts: ListenOptions, selection: ServerBackend.Selection): Promise<Listener> {
  log.info("server backend selected", {
    ...
    "jekko.server.runtime": HttpApiServer.name,
  })
