...
export function plan(input: { slug: string; time: { created: number } }, instance: InstanceContext) {
  const base = instance.project.vcs
    ? path.join(instance.worktree, ".jekko", "plans")
...
export class Service extends Context.Service<Service, Interface>()("@jekko/Session") {}
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("Session.create")(function* (input?: {
      ...
      return yield* createNext({
        parentID: input?.parentID,
        directory,
        path: input?.path ?? sessionPath(ctx.worktree, directory),
        title: input?.title,
        agent: input?.agent,
        model: input?.model,
        permission: input?.permission,
...
function* listByProject(
  ...
  } else if (input.scope !== "project" && !Flag.JEKKO_EXPERIMENTAL_WORKSPACES) {
    if (input.directory) {
