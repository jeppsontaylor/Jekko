This is a plugin to simulate a remote environment locally. Add this to `.jekko/jekko.jsonc`:

```json
  "plugin": ["../packages/jekko/src/control-plane/dev/debug-workspace-plugin.ts"],
```

In a separate terminal, run a separate Jekko server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/jekko/script/run-workspace-server
```

With the plugin install, you can now run Jekko and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
