...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for jekko.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal jekko sessions
...
```bash
JEKKO_ENABLE_QUESTION_TOOL=1 jekko acp
```
...
- **Terminal Integration**: Full terminal support via jekko's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | jekko acp
```
...
ACP sessions map cleanly to jekko's internal session model:
