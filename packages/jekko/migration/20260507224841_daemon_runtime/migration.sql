CREATE TABLE `daemon_run` (
	...
	`active_session_id` text NOT NULL REFERENCES `session`(`id`) ON DELETE cascade,
...
CREATE TABLE `daemon_iteration` (
	`run_id` text NOT NULL REFERENCES `daemon_run`(`id`) ON DELETE cascade,
	`iteration` integer NOT NULL,
...
CREATE TABLE `daemon_task_pass` (
	`id` text PRIMARY KEY NOT NULL,
	`run_id` text NOT NULL REFERENCES `daemon_run`(`id`) ON DELETE cascade,
	`task_id` text NOT NULL REFERENCES `daemon_task`(`id`) ON DELETE cascade,
...
CREATE TABLE `daemon_artifact` (
	`id` text PRIMARY KEY NOT NULL,
	`run_id` text NOT NULL REFERENCES `daemon_run`(`id`) ON DELETE cascade,
	`task_id` text REFERENCES `daemon_task`(`id`) ON DELETE cascade,
