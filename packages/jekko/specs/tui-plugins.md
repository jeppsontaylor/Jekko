...
- Author package entrypoint is `@jekko-ai/plugin/tui`.
...
```json
{
  "$schema": "https://jekko.ai/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/jekko-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@jekko-ai/plugin/tui`.
- `@jekko-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
- The value is a semver range checked against the running Jekko version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.jekko` is absent, no check is performed (backward compatible).
...
- `jekko plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `jekko plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.jekko` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.jekko`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.jekko`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.jekko` dir or `JEKKO_CONFIG_DIR`, Jekko installs `@jekko-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@jekko-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.jekko/themes` area near the plugin config source.
...
- `--pure` / `JEKKO_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.jekko/plugins/tui-smoke.tsx`
- Local vim plugin: `.jekko/plugins/tui-vim.tsx`
- Local smoke config: `.jekko/tui.json`
- Local smoke theme: `.jekko/plugins/smoke-theme.json`
