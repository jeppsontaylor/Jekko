...
Practical reference for extracting a future `packages/server` from the current `packages/jekko` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all jekko services, Effect-first source of truth
- `packages/server` - jekko server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full jekko server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/jekko`.
...
- those slices are mounted into the Hono server behind `JEKKO_EXPERIMENTAL_HTTPAPI`
...
The intended direction here is the same, but the current `jekko` package split is earlier in the migration.
...
- the current server host still lives in `packages/jekko`
...
- if `packages/server` imports services from `packages/jekko` while `packages/jekko` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/jekko` as the temporary Hono host
...
- `packages/server` must not import from `packages/jekko`

Allowed in phase 1:

- `packages/jekko` imports `packages/server`
...
- `packages/jekko` shrinks or disappears as package responsibilities are fully split
...
- `packages/jekko` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/jekko`
...
Keep in `packages/jekko` for now:
...
- host mounting from `packages/jekko`
...
- no imports from `packages/jekko` inside `packages/server`
...
- `packages/server` must still not import from `packages/jekko`
...
### PR 4. Mount `packages/server` question from `packages/jekko`

Scope:

- replace local experimental question route wiring in `packages/jekko`
...
- mount it in parallel from `packages/jekko`
...
- move adapter selection and server startup out of `packages/jekko`

Rules:

- do not start this while `packages/server` still depends on `packages/jekko`
...
- host logic no longer lives in `packages/jekko`
...
3. the route is mounted from the current host in `packages/jekko`
...
- `packages/server` imports services or runtime from `packages/jekko`
- `packages/jekko` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/jekko` imports
...
- [ ] mount `question` from `packages/jekko`
...
2. keep `packages/jekko` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/jekko`, stop and restructure the boundary first.
