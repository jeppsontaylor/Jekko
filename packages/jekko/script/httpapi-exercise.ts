 ...
 *
 * The script intentionally isolates `JEKKO_DB` before importing modules that touch
...
const preserveExerciseGlobalRoot = !!process.env.JEKKO_HTTPAPI_EXERCISE_GLOBAL
const exerciseGlobalRoot =
  process.env.JEKKO_HTTPAPI_EXERCISE_GLOBAL ??
  path.join(process.env.TMPDIR ?? "/tmp", `jekko-httpapi-global-${process.pid}`)
process.env.XDG_DATA_HOME = path.join(exerciseGlobalRoot, "data")
process.env.XDG_CONFIG_HOME = path.join(exerciseGlobalRoot, "config")
process.env.XDG_STATE_HOME = path.join(exerciseGlobalRoot, "state")
process.env.XDG_CACHE_HOME = path.join(exerciseGlobalRoot, "cache")
process.env.JEKKO_DISABLE_SHARE = "true"
const exerciseConfigDirectory = path.join(exerciseGlobalRoot, "config", "jekko")
const exerciseDataDirectory = path.join(exerciseGlobalRoot, "data", "jekko")

const preserveExerciseDatabase = !!process.env.JEKKO_HTTPAPI_EXERCISE_DB
const exerciseDatabasePath =
  process.env.JEKKO_HTTPAPI_EXERCISE_DB ??
  path.join(process.env.TMPDIR ?? "/tmp", `jekko-httpapi-exercise-${process.pid}.db`)
process.env.JEKKO_DB = exerciseDatabasePath
Flag.JEKKO_DB = exerciseDatabasePath

void (await import("@jekko-ai/core/util/log")).init({ print: false })
...
const scenarios: Scenario[] = [
  ...
  http
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "jekko.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "jekko.jsonc")).text(),
          )
  ...
  http.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-jekko-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-jekko-directory")
  }),
  ...
  http
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "jekko", model: "test" })}`,
      headers: ctx.headers(),
    }))
    ...
    Effect.flatMap((context) =>
      Effect.gen(function* () {
        ...
        const base: ScenarioContext = {
          ...
          headers: (extra) => ({
            ...(context.dir?.path ? { "x-jekko-directory": context.dir.path } : {}),
            ...extra,
          }),
          file: (name, content) =>
            Effect.promise(() => {
...
function app(modules: Runtime, backend: Backend) {
  Flag.JEKKO_EXPERIMENTAL_HTTPAPI = backend === "effect"
    ...
    modules.ExperimentalHttpApiServer.routes.pipe(
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({ JEKKO_SERVER_PASSWORD: undefined, JEKKO_SERVER_USERNAME: undefined }),
        ),
      ),
    ),
    { disableLogger: true },
