...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.jekko.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `jekko.settings.dat` is special: it maps to the `jekko.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"jekko.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // jekko.settings.dat → the electron settings store ("jekko.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "jekko.global.dat").
  const storeName = filename === "jekko.settings.dat" ? "jekko.settings" : filename
