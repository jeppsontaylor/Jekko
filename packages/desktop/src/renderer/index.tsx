...
import "./styles.css"
import { useTheme } from "@jekko-ai/ui/theme"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.JEKKO_CHANNEL === "prod" &&
...
const deepLinkEvent = "jekko:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__JEKKO__ ??= {}
  const pending = window.__JEKKO__.deepLinks ?? []
  window.__JEKKO__.deepLinks = [...pending, ...urls]
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://jekko.ai/favicon-96x96.png",
      })
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("jekko.global.dat").getItem("language")
    const historical = current ? undefined : await platform.storage?.().getItem("language.v1")
