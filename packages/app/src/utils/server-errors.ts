...
export function formatServerError(error: unknown, translate?: Translator, alternative_path?: string) {
  ...
  if (alternative_path) return alternative_path
...
function parseReadableProviderModelNotFoundError(errorInput: ProviderModelNotFoundError, translator?: Translator) {
  ...
  const tail = tr(translator, "error.chain.checkConfig", "Check your config (jekko.json) provider/model names")
  if (list.length) {
