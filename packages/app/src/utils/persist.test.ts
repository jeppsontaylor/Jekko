...
class MemoryStorage implements Storage {
  ...
  getItem(key: string) {
    ...
    if (key.startsWith("jekko.throw")) throw new Error("storage get failed")
  ...
  setItem(key: string, value: string) {
    ...
    if (key.startsWith("jekko.quota")) throw new DOMException("quota", "QuotaExceededError")
    if (key.startsWith("jekko.throw")) throw new Error("storage set failed")
  ...
  removeItem(key: string) {
    ...
    if (key.startsWith("jekko.throw")) throw new Error("storage remove failed")
...
describe("persist localStorage resilience", () => {
  test("does not cache values as persisted when quota write and eviction fail", () => {
    const storageApi = persistTesting.localStorageWithPrefix("jekko.quota.scope")
    storageApi.setItem("value", '{"value":1}')

    expect(storage.getItem("jekko.quota.scope:value")).toBeNull()
    expect(storageApi.getItem("value")).toBeNull()
  })

  test("disables only the failing scope when storage throws", () => {
    const bad = persistTesting.localStorageWithPrefix("jekko.throw.scope")
    ...
    const healthy = persistTesting.localStorageWithPrefix("jekko.safe.scope")
    healthy.setItem("value", '{"value":3}')
    expect(storage.getItem("jekko.safe.scope:value")).toBe('{"value":3}')
  })

  test("failing alternative_path scope does not poison direct storage scope", () => {
    const broken = persistTesting.localStorageWithPrefix("jekko.throw.scope2")
  ...
  test("workspace storage sanitizes Windows filename characters", () => {
    ...
    expect(result).toStartWith("jekko.workspace.")
