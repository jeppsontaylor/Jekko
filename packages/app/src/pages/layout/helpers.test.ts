...
import { type Session } from "@jekko-ai/sdk/v2/client"
import {
  childSessionOnPath,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("jekko://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("jekko://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("jekko://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("jekko://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("jekko://open-project")).toBeUndefined()
    expect(parseDeepLink("jekko://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "jekko://open-project?directory=/a",
      "jekko://other?directory=/b",
      "jekko://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("jekko://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("jekko://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("jekko://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("jekko://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "jekko://new-session?directory=/a",
      "jekko://open-project?directory=/b",
      "jekko://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __JEKKO__: {
        deepLinks: ["jekko://open-project?directory=/a"],
      },
    } as unknown as Window & { __JEKKO__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["jekko://open-project?directory=/a"])
