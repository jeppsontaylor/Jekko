...
Las herramientas permiten que LLM realice acciones en su código base. Jekko viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Jekko.
...
:::note
Esta herramienta solo está disponible cuando `JEKKO_EXPERIMENTAL_LSP_TOOL=true` (o `JEKKO_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Jekko o cuando la variable de entorno `JEKKO_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Jekko:

```bash
JEKKO_ENABLE_EXA=1 jekko
```
