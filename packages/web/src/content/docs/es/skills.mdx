...
Las habilidades del agente permiten a Jekko descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Jekko busca estas ubicaciones:

- Configuración del proyecto: `.jekko/skills/<name>/SKILL.md`
- Configuración global: `~/.config/jekko/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Jekko sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.jekko/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/jekko/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.jekko/skills/git-release/SKILL.md` así:
...
Jekko enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `jekko.json`:
...
**Para agentes integrados** (en `jekko.json`):
