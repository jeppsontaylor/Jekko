...
Con Jekko puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Jekko usa nuestro propio tema `jekko`.
...
Jekko viene con varios temas integrados.
...
- Quiere que Jekko coincida con la apariencia de su terminal
...
Jekko admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/jekko/themes/*.json` o `$XDG_CONFIG_HOME/jekko/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.jekko/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.jekko/themes/*.json`
