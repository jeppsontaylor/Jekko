...
description: Cree herramientas que LLM pueda llamar en jekko.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de jekko como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.jekko/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/jekko/tools/`.
...
```ts title=".jekko/tools/database.ts" {1}
...
```ts title=".jekko/tools/project.ts" {8}
import { tool } from "@jekko-ai/plugin"
...
```python title=".jekko/tools/add.py"
import sys
...
```ts title=".jekko/tools/python-add.ts" {10}
import { tool } from "@jekko-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".jekko/tools/add.py")
