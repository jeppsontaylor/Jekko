---
title: Complementos
description: Escriba sus propios complementos para extender Jekko.
---

Los complementos le permiten extender Jekko al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Jekko.
...
- `.jekko/plugins/` - Complementos a nivel de proyecto
- `~/.config/jekko/plugins/` - Complementos globales
...
```json title="jekko.json"
{
  ...
  "plugin": ["jekko-helicone-session", "jekko-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/jekko/node_modules/`.
...
1. Configuración global (`~/.config/jekko/jekko.json`)
2. Configuración del proyecto (`jekko.json`)
3. Directorio global de complementos (`~/.config/jekko/plugins/`)
4. Directorio de complementos del proyecto (`.jekko/plugins/`)
...
Jekko ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".jekko/plugins/my-plugin.ts"
...
```js title=".jekko/plugins/example.js"
...
- `client`: Un cliente SDK jekko para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar jekko.
...
```js title=".jekko/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "jekko"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Jekko, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que jekko lea archivos `.env`:

```javascript title=".jekko/plugins/env-protection.js"
...
```javascript title=".jekko/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a jekko:

```ts title=".jekko/plugins/custom-tools.ts"
import { type Plugin, tool } from "@jekko-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que jekko puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para jekko junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://jekko.ai/docs/sdk) para obtener más detalles.
...
```ts title=".jekko/plugins/compaction.ts"
import type { Plugin } from "@jekko-ai/plugin"
...
```ts title=".jekko/plugins/custom-compaction.ts"
import type { Plugin } from "@jekko-ai/plugin"
