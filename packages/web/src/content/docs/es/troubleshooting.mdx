...
Para depurar problemas con Jekko, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/jekko/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\jekko\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `jekko --log-level DEBUG`.

---

## Almacenamiento

jekko almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/jekko/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\jekko`
...
Jekko Desktop ejecuta un servidor Jekko local (el sidecar `jekko-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Jekko` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/jekko/jekko.jsonc` (o `~/.config/jekko/jekko.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/jekko/jekko.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\jekko\jekko.jsonc`
...
Jekko también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/jekko/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\jekko\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.jekko/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Jekko pueda reconstruirlo.

1. Salga de Jekko Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/jekko`
- **Linux**: eliminar `~/.cache/jekko` (o ejecutar `rm -rf ~/.cache/jekko`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\jekko`

3. Reinicie el escritorio Jekko.

---

### Solucionar problemas de conexión del servidor

Jekko Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `jekko.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `JEKKO_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `JEKKO_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Jekko requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Jekko.

---

### Notificaciones que no se muestran

Jekko Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Jekko en la configuración de su sistema operativo, y
...
1. Salga del escritorio Jekko.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Jekko):

- `jekko.settings.dat` (URL del servidor predeterminado de escritorio)
- `jekko.global.dat` y `jekko.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/jekko/issues**](https://github.com/anomalyco/jekko/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**jekko.ai/discord**](https://jekko.ai/discord)
...
3. Asegúrese de tener la última versión con `jekko upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `jekko models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/jekko
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\jekko`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. jekko instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\jekko`

2. Reinicie jekko para reinstalar los paquetes del proveedor más recientes.

Esto obligará a jekko a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
jekko detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
