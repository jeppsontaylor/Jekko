---
title: Servidor
description: Interactuar con el servidor jekko a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `jekko serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente jekko puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `jekko.local` |
...
Configure `JEKKO_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `jekko`, o configure `JEKKO_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `jekko serve` como a `jekko web`.
...
Cuando ejecuta `jekko`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor jekko para interactuar con jekko mediante programación.
:::

Esta arquitectura permite que jekko admita múltiples clientes y le permite interactuar con jekko mediante programación.

Puede ejecutar `jekko serve` para iniciar un servidor independiente. Si tienes el
jekko TUI ejecutándose, `jekko serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Jekko [IDE](/docs/ide).
...
El servidor jekko expone las siguientes API.
