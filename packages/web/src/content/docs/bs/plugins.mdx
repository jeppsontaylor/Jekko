---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Jekko.
---

Dodaci vam omogućavaju da proširite Jekko spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Jekko.
...
- `.jekko/plugins/` - Dodaci na nivou projekta
- `~/.config/jekko/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/jekko/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/jekko/jekko.json`)
2. Konfiguracija projekta (`jekko.json`)
3. Globalni direktorij dodataka (`~/.config/jekko/plugins/`)
4. Direktorij dodataka projekta (`.jekko/plugins/`)
...
Jekko pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Jekko.
...
Spriječite jekko da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u jekko:
...
Pomoćnik `tool` kreira prilagođeni alat koji jekko može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za jekko zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://jekko.ai/docs/sdk) za detalje.
