...
Da biste otklonili probleme s Jekko, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\jekko\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `jekko --log-level DEBUG`.

---

## Pohrana

Jekko pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/jekko/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\jekko`
...
Jekko Desktop pokreće lokalni Jekko server (`jekko-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Jekko` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/jekko/jekko.jsonc` (ili `~/.config/jekko/jekko.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/jekko/jekko.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\jekko\jekko.jsonc`
...
Jekko također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/jekko/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\jekko\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Jekko mogao ponovo izgraditi.

1. Potpuno zatvorite Jekko Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/jekko`
- **Linux**: obrišite `~/.cache/jekko` (ili pokrenite `rm -rf ~/.cache/jekko`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\jekko`

3. Ponovo pokrenite Jekko Desktop.

---

### Rješavanje problema sa vezom na serveru

Jekko Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `jekko.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `JEKKO_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `JEKKO_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Jekko Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Jekko karakteristikama.

---

### Obavještenja se ne prikazuju

Jekko Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Jekko u postavkama vašeg OS-a, i
...
1. Zatvorite Jekko Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Jekko Desktop aplikacije):

- `jekko.settings.dat` (URL zadanog servera za desktop)
- `jekko.global.dat` i `jekko.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `jekko upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `jekko models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\jekko`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Jekko dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\jekko`

2. Ponovo pokrenite Jekko da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Jekko da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Jekko će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
