...
Možete dodati vanjske alate u Jekko koristeći _Model Context Protocol_, ili MCP. Jekko podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Jekko Config](https://jekko.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/jekko`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Jekko automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Jekko će:
...
Ako server zahtijeva autentifikaciju, Jekko će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `jekko mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Jekko sigurno čuva tokene u `~/.local/share/jekko/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Jekko, zajedno s ugrađenim alatima. Možete njima upravljati kroz Jekko konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Jekko sa vašim Sentry nalogom.
