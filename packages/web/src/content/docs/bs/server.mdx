---
title: Server
description: Komunicirajte s jekko serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `jekko serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji jekko klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `jekko.local` |
...
Postavite `JEKKO_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `jekko`, ili postavite `JEKKO_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `jekko serve` i za `jekko web`.
...
Kada pokrenete `jekko`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite jekko server za programsku interakciju sa jekko-om.
:::

Ova arhitektura omogucava jekko podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `jekko serve` da startate standalone server. Ako je jekko TUI vec pokrenut, `jekko serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Jekko [IDE](/docs/ide) pluginovi.
...
jekko server izlaže sljedece API-je.
