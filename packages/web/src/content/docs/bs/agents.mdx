...
Postoje dvije vrste agenata u Jekko: primarni agenti i podagenti.
...
Jekko dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Jekko dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Jekko dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `jekko.json`:
...
- Globalno: `~/.config/jekko/agents/`
- Po projektu: `.jekko/agents/`
...
Ako temperatura nije navedena, Jekko koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Jekko konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Jekko konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Jekko Zen](/docs/zen), koristili biste `jekko/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `jekko models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/jekko).
:::
