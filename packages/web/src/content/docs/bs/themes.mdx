...
U Jekko mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Jekko koristi nasu `jekko` temu.
...
Jekko dolazi sa vise ugradenih tema.
...
- Zele da Jekko odgovara izgledu njihovog terminala
...
Jekko podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/jekko/themes/*.json` ili `$XDG_CONFIG_HOME/jekko/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.jekko/themes/*.json`
4. **Trenutni radni direktorij** - `./.jekko/themes/*.json`
