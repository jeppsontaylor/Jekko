...
Agent skills omogucavaju Jekko da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Jekko pretrazuje ove lokacije:

- Konfiguracija projekta: `.jekko/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/jekko/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Jekko ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.jekko/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/jekko/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.jekko/skills/git-release/SKILL.md` ovako:
...
Jekko navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `jekko.json`:
...
**Za ugradene agente** (u `jekko.json`):
