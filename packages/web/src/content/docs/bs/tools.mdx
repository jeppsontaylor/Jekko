...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Jekko dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Jekko.
...
:::note
Ovaj alat je dostupan samo kada je `JEKKO_EXPERIMENTAL_LSP_TOOL=true` (ili `JEKKO_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Jekko provajdera ili kada je varijabla `JEKKO_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Jekko:
