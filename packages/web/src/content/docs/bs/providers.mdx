---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Jekko.
---

import config from "../../../../config.mjs"
export const console = config.console

Jekko koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Jekko konfiguraciji.
...
u `~/.local/share/jekko/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Jekko
config.
...
Jekko Zen je lista modela koje je obezbedio Jekko tim koji su bili
testirano i potvrđeno da dobro radi sa Jekko. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Jekko Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite jekko i idite na [jekko.ai/auth](https://jekko.ai/auth).
...
Radi kao i svaki drugi provajder u Jekko i potpuno je opcionalan za korištenje.

---

## Jekko Go

Jekko Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Jekko tim i koji su testirani i verificirani da dobro rade s Jekko-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Jekko Go` i idite na [jekko.ai/auth](https://jekko.ai/zen).
...
Da biste koristili Amazon Bedrock s Jekko:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete jekko:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `jekko.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Jekko.
:::
...
Možete konfigurirati jekko za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi jekko ispravno radio.
   :::
...
Također možete dodati modele kroz svoju jekko konfiguraciju.
...
da pohrani token u jekko auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Jekko koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Jekko
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`jekko.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [jekko-gitlab-auth](https://www.npmjs.com/package/jekko-gitlab-auth).

##### Konfiguracija

Prilagodite putem `jekko.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s jekko:
...
Za korištenje Google Vertex AI s Jekko:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće jekko.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem jekkoa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [jekko-helicone-session](https://github.com/H2Shami/jekko-helicone-session) da automatski prijavite svaki Jekko razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Jekko razgovor naveden kao zasebna sesija.
...
Možete konfigurirati jekko za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati jekko za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati jekko za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Jekko. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/jekko) za detalje.
:::
...
Da biste koristili Ollama Cloud s Jekko:
...
3. Kopirajte API ključ za korištenje u Jekko.
...
6. **Važno**: Prije upotrebe modela oblaka u Jekko, morate lokalno povući informacije o modelu:
...
Jekko Zen je lista testiranih i verifikovanih modela koju je obezbedio Jekko tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Jekko Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Jekko Zen**.

```txt
   /connect
```

3. Unesite svoj Jekko API ključ.
...
Također možete dodati dodatne modele putem vaše jekko konfiguracije.
...
5. Također ih možete prilagoditi putem vaše jekko konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju jekko konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s jekko-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `jekko.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Jekko da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `jekko auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Jekko konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj jekko konfiguraciji.
