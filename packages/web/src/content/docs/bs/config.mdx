---
title: Konfiguracija
description: Korištenje Jekko JSON konfiguracije.
---

Možete konfigurirati Jekko koristeći JSON konfiguracijski fajl.

---

## Format

Jekko podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/jekko`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/jekko/jekko.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`JEKKO_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`jekko.json` u projektu) - postavke specifične za projekat
5. **`.jekko` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`JEKKO_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.jekko` i `~/.config/jekko` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/jekko` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Jekko konfiguraciju u `~/.config/jekko/jekko.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/jekko/tui.json`.
...
Dodajte `jekko.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Jekko pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `JEKKO_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `JEKKO_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.jekko` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.jekko`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`jekko.ai/config.json`**](https://jekko.ai/config.json).

TUI konfiguracija koristi [**`jekko.ai/tui.json`**](https://jekko.ai/tui.json).
...
Koristite `JEKKO_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `jekko.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `jekko serve` i `jekko web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Jekko server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `jekko.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Jekko konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Jekko pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/jekko/agents/` ili `.jekko/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Jekko će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`jekko run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/jekko/commands/` ili `.jekko/commands/`. [Saznajte više ovdje](/docs/commands).
...
Jekko će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Jekko **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Jekko sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.jekko/plugins/` ili `~/.config/jekko/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Jekko da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
