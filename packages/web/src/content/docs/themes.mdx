...
With Jekko you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Jekko uses our own `jekko` theme.
...
Jekko comes with several built-in themes.
...
- Want Jekko to match their terminal's appearance
...
Jekko supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/jekko/themes/*.json` or `$XDG_CONFIG_HOME/jekko/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.jekko/themes/*.json`
4. **Current working directory** - Defined in `./.jekko/themes/*.json`
