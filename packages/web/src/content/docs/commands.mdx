...
You can add custom commands through the Jekko config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Jekko [config](/docs/config):
...
jekko includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
