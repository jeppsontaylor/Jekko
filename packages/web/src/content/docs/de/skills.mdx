...
Agent Skills erlauben Jekko, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Jekko sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Jekko vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.jekko/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/jekko/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.jekko/skills/git-release/SKILL.md` zum Beispiel so:
...
Jekko listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `jekko.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `jekko.json`):
