...
Mit Jekko kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Jekko das `jekko`-Theme.
...
Jekko liefert mehrere eingebaute Themes mit.
...
- Jekko optisch ans Terminal anpassen wollen
...
Jekko unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/jekko/themes/*.json` oder `$XDG_CONFIG_HOME/jekko/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.jekko/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.jekko/themes/*.json`
