...
Wenn Jekko Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\jekko\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `jekko --log-level DEBUG`.

---

## Speicher

jekko speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/jekko/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\jekko`
...
Jekko Desktop startet im Hintergrund einen lokalen Jekko-Server (`jekko-cli`-Sidecar).
...
- Nur macOS: `Jekko`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/jekko/jekko.jsonc` (oder `~/.config/jekko/jekko.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/jekko/jekko.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\jekko\jekko.jsonc`
...
Jekko kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/jekko/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\jekko\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Jekko ihn neu aufbauen kann.

1. Beende Jekko Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/jekko`
- **Linux**: loesche `~/.cache/jekko` (oder fuehre aus: `rm -rf ~/.cache/jekko`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\jekko`

3. Starte Jekko Desktop neu.

---

### Server-Verbindungsprobleme beheben

Jekko Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `jekko.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `JEKKO_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `JEKKO_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Jekko Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Jekko-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Jekko Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Jekko in den OS-Einstellungen aktiviert sind, und
...
1. Beende Jekko Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Jekko Desktop):

- `jekko.settings.dat` (desktop default server URL)
- `jekko.global.dat` und `jekko.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Jekko hast:
...
3. Stelle sicher, dass du die neueste Version hast: `jekko upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `jekko models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\jekko`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. jekko installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\jekko`

2. Starte jekko neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt jekko, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
jekko erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
