...
Sie können externe Tools zu Jekko hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Jekko unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Jekko Config](https://jekko.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/jekko`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Jekko übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Jekko Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Jekko beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `jekko mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Jekko die Token sicher in `~/.local/share/jekko/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Jekko verfügbar. Sie können sie also wie jedes andere Tool über die Jekko-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Jekko mit Ihrem Sentry-Konto zu verbinden.
