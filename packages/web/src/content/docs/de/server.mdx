---
title: Server
description: Nutze den jekko-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `jekko serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein jekko-Client nutzen kann.
...
Setze `JEKKO_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `jekko`, kann aber mit `JEKKO_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `jekko serve` und `jekko web`.
...
Wenn du `jekko` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den jekko-Server, um jekko programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `jekko serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `jekko serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Jekko-[IDE](/docs/ide)-Plugins.
...
Der jekko-Server stellt folgende APIs bereit.
