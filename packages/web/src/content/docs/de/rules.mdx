---
title: Regeln
description: Lege eigene Anweisungen fuer jekko fest.
---

Du kannst jekko ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in jekko den Befehl `/init` aus.
...
So kann jekko sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht jekko sie zu erweitern.
...
jekko kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/jekko/AGENTS.md` speichern.
Diese gelten in allen jekko-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Jekko dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/jekko/AGENTS.md` existiert)
...
Beim Start sucht jekko in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/jekko/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `jekko.json` oder in `~/.config/jekko/jekko.json` angeben.
...
jekko parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit jekko.json

Empfohlen ist das `instructions`-Feld in der `jekko.json`:
...
Du kannst jekko explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `jekko.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
