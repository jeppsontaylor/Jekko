---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Jekko zu erweitern.
---

Mit Plugins können Sie Jekko erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Jekko zu ändern.
...
- `.jekko/plugins/` – Plugins auf Projektebene
- `~/.config/jekko/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/jekko/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/jekko/jekko.json`)
2. Projektkonfiguration (`jekko.json`)
3. Globales Plugin-Verzeichnis (`~/.config/jekko/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.jekko/plugins/`)
...
Jekko führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Jekko-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Jekko erweitern können.
...
:::note
Wenn Sie die Jekko-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Jekko `.env`-Dateien liest:
...
Plugins können Jekko auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Jekko aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Jekko zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://jekko.ai/docs/sdk).
