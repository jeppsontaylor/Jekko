---
title: TUI
description: Using the Jekko terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Jekko provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Jekko starts the TUI for the current directory.
...
When using the Jekko TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Jekko. Allows you to select from available providers and add their API keys.
...
This is separate from `jekko.json`, which configures server/runtime behavior.
...
Use `JEKKO_TUI_CONFIG` to load a custom TUI config path.
