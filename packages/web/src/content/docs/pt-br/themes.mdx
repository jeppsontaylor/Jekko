...
Com o jekko, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o jekko usa nosso próprio tema `jekko`.
...
O jekko vem com vários temas embutidos.
...
- Querem que o jekko corresponda à aparência do seu terminal
...
O jekko suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/jekko/themes/*.json` ou `$XDG_CONFIG_HOME/jekko/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.jekko/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.jekko/themes/*.json`
