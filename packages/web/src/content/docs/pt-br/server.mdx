---
title: Servidor
description: Interaja com o servidor jekko via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `jekko serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente jekko pode usar.
...
```bash
jekko serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `jekko.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
jekko serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `JEKKO_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `jekko`, ou defina `JEKKO_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `jekko serve` quanto ao `jekko web`.

```bash
JEKKO_SERVER_PASSWORD=your-password jekko serve
```
...
Quando você executa `jekko`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor jekko para interagir com o jekko programaticamente.
:::

Essa arquitetura permite que o jekko suporte múltiplos clientes e permite que você interaja com o jekko programaticamente.

Você pode executar `jekko serve` para iniciar um servidor autônomo. Se você tiver o TUI do jekko em execução, `jekko serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do jekko [IDE](/docs/ide).
...
O servidor jekko expõe as seguintes APIs.
