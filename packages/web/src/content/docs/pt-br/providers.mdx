---
title: Provedores
description: Usando qualquer provedor LLM no jekko.
---

import config from "../../../../config.mjs"
export const console = config.console

jekko usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do jekko.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/jekko/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do jekko.
...
Jekko Zen é uma lista de modelos fornecidos pela equipe do jekko que foram testados e verificados para funcionar bem com o jekko. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o Jekko Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `Jekko Zen` e acesse [jekko.ai/zen](https://jekko.ai/zen).
...
Funciona como qualquer outro provedor no jekko e é completamente opcional.

---

## Jekko Go

Jekko Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do jekko que foram
testados e verificados para funcionar bem com o jekko.

1. Execute o comando `/connect` no TUI, selecione `Jekko Go` e acesse [jekko.ai/zen](https://jekko.ai/zen).
...
Para usar o Amazon Bedrock com o jekko:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o jekko:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `jekko.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no jekko não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o jekko para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o jekko funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do jekko.
...
armazenar o token no armazenamento de autenticação do jekko.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
jekko usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o jekko
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `jekko.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Mais documentação na página [jekko-gitlab-auth](https://www.npmjs.com/package/jekko-gitlab-auth).

##### Configuração

Personalize através do `jekko.json`:
...
Para usar sua assinatura do GitHub Copilot com o jekko:
...
Para usar o Google Vertex AI com o jekko:
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do jekko, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [jekko-helicone-session](https://github.com/H2Shami/jekko-helicone-session) para registrar automaticamente cada conversa do jekko como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do jekko listada como uma sessão separada.
...
Você pode configurar o jekko para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o jekko para usar modelos locais através do LM Studio.
...
Você pode configurar o jekko para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o jekko. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/jekko) para detalhes.
:::
...
Para usar o Ollama Cloud com o jekko:
...
3. Copie a chave da API para uso no jekko.
...
6. **Importante**: Antes de usar modelos em nuvem no jekko, você deve puxar as informações do modelo localmente:
...
Jekko Zen é uma lista de modelos testados e verificados fornecidos pela equipe do jekko. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>Jekko Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **Jekko Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do jekko.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do jekko.
...
5. Você também pode personalizá-los através da sua configuração do jekko. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o jekko:
...
Você também pode personalizar modelos através da sua configuração do jekko. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o jekko. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no jekko.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `jekko.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o jekko entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `jekko auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do jekko e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do jekko.
