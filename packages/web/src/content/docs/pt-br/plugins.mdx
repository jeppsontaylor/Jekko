---
title: Plugins
description: Escreva seus próprios plugins para estender o jekko.
---

Plugins permitem que você estenda o jekko conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do jekko.
...
- `.jekko/plugins/` - Plugins em nível de projeto
- `~/.config/jekko/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/jekko/node_modules/`.
...
1. Configuração global (`~/.config/jekko/jekko.json`)
2. Configuração do projeto (`jekko.json`)
3. Diretório de plugins global (`~/.config/jekko/plugins/`)
4. Diretório de plugins do projeto (`.jekko/plugins/`)
...
O jekko executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do jekko para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o jekko.
...
:::note
Se você estiver usando o aplicativo desktop jekko, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o jekko de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao jekko:
...
O helper `tool` cria uma ferramenta personalizada que o jekko pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o jekko junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://jekko.ai/docs/sdk) para detalhes.
