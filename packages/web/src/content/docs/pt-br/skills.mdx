...
As habilidades do agente permitem que o jekko descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O jekko pesquisa nesses locais:

- Configuração do projeto: `.jekko/skills/<name>/SKILL.md`
- Configuração global: `~/.config/jekko/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o jekko sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.jekko/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/jekko/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.jekko/skills/git-release/SKILL.md` assim:
...
O jekko lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `jekko.json`:
...
**Para agentes embutidos** (em `jekko.json`):
