...
Создайте `.jekko/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию jekko или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем jekko [config](/docs/config):
...
- Глобальный: `~/.config/jekko/commands/`
- Для каждого проекта: `.jekko/commands/`
...
jekko включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
