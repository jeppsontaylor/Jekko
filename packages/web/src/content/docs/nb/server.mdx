---
title: Tjener
description: Samhandle med Jekko-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `jekko serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Jekko-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `jekko.local` |
...
Sett `JEKKO_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `jekko` som standard, eller sett `JEKKO_SERVER_USERNAME` for å overstyre det. Dette gjelder både `jekko serve` og `jekko web`.
...
Når du kjører `jekko` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Jekko-serveren til å samhandle med Jekko programmatisk.
:::

Denne arkitekturen lar Jekko støtte flere klienter og lar deg samhandle med Jekko programmatisk.

Du kan kjøre `jekko serve` for å starte en frittstående server. Hvis du har
Jekko TUI kjører, vil `jekko serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Jekko [IDE](/docs/ide) plugins.
...
jekko-serveren viser følgende APIer.
