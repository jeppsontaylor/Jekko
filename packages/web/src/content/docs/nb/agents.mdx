...
Det er to typer agenter i Jekko; primære agenter og underagenter.
...
Jekko kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Jekko kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

Jekko kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `jekko.json`:
...
- Globalt: `~/.config/jekko/agents/`
- Per prosjekt: `.jekko/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Jekko modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Jekko-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Jekko-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Jekko Zen](/docs/zen), vil du bruke `jekko/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `jekko models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/jekko).
:::
