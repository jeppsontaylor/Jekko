...
Du kan legge til eksterne verktøy i Jekko ved å bruke _Model Context Protocol_ eller MCP. Jekko støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Jekko Config](https://jekko.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="jekko.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/jekko`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Jekko håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Jekko:
...
Hvis serveren krever autentisering, vil Jekko be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `jekko mcp auth <server-name>`.
...
```bash
jekko mcp auth my-oauth-server
```
...
```bash
jekko mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Jekko lagre tokens sikkert i `~/.local/share/jekko/mcp-auth.json`.
...
jekko mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Jekko, sammen med innebygde verktøy. Så du kan administrere dem gjennom Jekko-konfigurasjonen som alle andre verktøy.
...
```json title="jekko.json" {11, 14-18}
{
  "$schema": "https://jekko.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
jekko mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Jekko til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Jekko-samtale oppført som en separat økt.
