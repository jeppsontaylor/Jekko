---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Jekko.
---

import config from "../../../../config.mjs"
export const console = config.console

Jekko bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Jekko-konfigurasjonen.
...
i `~/.local/share/jekko/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Jekko
konfig.
...
Jekko Zen er en liste over modeller levert av Jekko-teamet som har vært
testet og verifisert for å fungere godt med Jekko. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Jekko Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Jekko Zen` og gå til [jekko.ai/auth](https://jekko.ai/zen).
...
Det fungerer som alle andre leverandører i Jekko og er helt valgfritt å bruke.

---

## Jekko Go

Jekko Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Jekko-teamet som har vært
testet og verifisert for å fungere godt med Jekko.

1. Kjør kommandoen `/connect` i TUI, velg `Jekko Go`, og gå til [jekko.ai/auth](https://jekko.ai/zen).
...
Slik bruker du Amazon Bedrock med Jekko:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Jekko:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `jekko.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i jekko støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere jekko til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Jekko skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Jekko-konfigurasjonen.
...
for å lagre token i Jekko auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Jekko bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Jekko
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`jekko.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [jekko-gitlab-auth](https://www.npmjs.com/package/jekko-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `jekko.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Jekko:
...
Slik bruker du Google Vertex AI med Jekko:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Jekko, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [jekko-helicone-session](https://github.com/H2Shami/jekko-helicone-session) plugin for automatisk å logge hver Jekko-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Jekko-samtale oppført som en separat økt.
...
Du kan konfigurere Jekko for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Jekko for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Jekko for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Jekko. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/jekko) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Jekko:
...
3. Kopier API-nøkkelen for bruk i Jekko.
...
6. **Viktig**: Før du bruker skymodeller i Jekko, må du hente modellinformasjonen lokalt:
...
Jekko Zen er en liste over testede og verifiserte modeller levert av Jekko-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Jekko Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Jekko Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Jekko API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Jekko-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Jekko-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Jekko:
...
Du kan også tilpasse modeller gjennom Jekko-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Jekko. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `jekko.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Jekko forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `jekko auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Jekko-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Jekko-konfigurasjonen.
