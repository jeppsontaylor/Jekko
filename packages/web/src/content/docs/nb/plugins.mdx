...
description: Skriv dine egne programtillegg for å utvide Jekko.
---

Plugins lar deg utvide Jekko ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Jekko.
...
- `.jekko/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/jekko/node_modules/`.
...
1. Global konfigurasjon (`~/.config/jekko/jekko.json`)
2. Prosjektkonfigurasjon (`jekko.json`)
3. Global plugin-katalog (`~/.config/jekko/plugins/`)
4. Prosjektpluginkatalog (`.jekko/plugins/`)
...
Jekko kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Jekko SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Jekko.
...
:::note
Hvis du bruker Jekko-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Jekko fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Jekko:
...
`tool`-hjelperen lager et tilpasset verktøy som Jekko kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Jekko sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://jekko.ai/docs/sdk) for detaljer.
