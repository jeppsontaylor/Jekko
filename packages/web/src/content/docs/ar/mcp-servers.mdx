...
يمكنك إضافة أدوات خارجية إلى Jekko باستخدام _Model Context Protocol_، أو MCP. يدعم Jekko خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Jekko](https://jekko.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/jekko` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Jekko تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Jekko بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Jekko المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `jekko mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Jekko الرموز بشكل آمن في `~/.local/share/jekko/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Jekko إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Jekko مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Jekko بحساب Sentry الخاص بك.
