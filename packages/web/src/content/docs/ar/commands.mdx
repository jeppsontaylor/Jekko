...
أنشئ الملف `.jekko/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Jekko أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Jekko:
...
- على مستوى النظام: `~/.config/jekko/commands/`
- لكل مشروع: `.jekko/commands/`
...
يتضمن jekko عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
