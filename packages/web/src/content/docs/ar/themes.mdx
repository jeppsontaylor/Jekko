...
مع Jekko يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Jekko سمتنا `jekko`.
...
- يريدون أن يطابق Jekko مظهر terminal لديهم
...
يدعم Jekko نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/jekko/themes/*.json` أو `$XDG_CONFIG_HOME/jekko/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.jekko/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.jekko/themes/*.json`
