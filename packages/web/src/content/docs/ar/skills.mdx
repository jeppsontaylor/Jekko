...
تتيح مهارات الوكيل لـ Jekko اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Jekko في هذه المواقع:

- إعدادات المشروع: `.jekko/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/jekko/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Jekko صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.jekko/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/jekko/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.jekko/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Jekko المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `jekko.json`:
...
**للوكلاء المدمجين** (في `jekko.json`):
