---
title: المزوّدون
description: استخدام أي موفّر LLM في Jekko.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Jekko ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Jekko.
...
في `~/.local/share/jekko/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Jekko.
...
Jekko Zen هي قائمة نماذج يوفّرها فريق Jekko وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Jekko. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Jekko Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر jekko، ثم انتقل إلى [jekko.ai/auth](https://jekko.ai/auth).
...
يعمل مثل أي مزوّد آخر في Jekko واستخدامه اختياري بالكامل.

---

## Jekko Go

Jekko Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Jekko، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Jekko.

1. شغّل الأمر `/connect` في TUI، واختر `Jekko Go`، ثم انتقل إلى [jekko.ai/auth](https://jekko.ai/zen).
...
لاستخدام Amazon Bedrock مع Jekko:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل jekko:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `jekko.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Jekko غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين jekko لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل jekko بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات jekko.
...
تخزين الرمز في مساحة تخزين مصادقة jekko.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Jekko نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Jekko
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`jekko.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [jekko-gitlab-auth](https://www.npmjs.com/package/jekko-gitlab-auth).
...
لاستخدام اشتراك GitHub Copilot مع jekko:
...
لاستخدام Google Vertex AI مع Jekko:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر jekko، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [jekko-helicone-session](https://github.com/H2Shami/jekko-helicone-session) لتسجيل كل محادثة Jekko تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Jekko مدرجة كجلسة مستقلة.
...
يمكنك ضبط jekko لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط jekko لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط jekko لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Jekko. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/jekko) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Jekko:
...
3. انسخ مفتاح API لاستخدامه في Jekko.
...
6. **مهم**: قبل استخدام نماذج السحابة في Jekko، يجب سحب معلومات النموذج محليا:
...
Jekko Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Jekko. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Jekko Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Jekko Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Jekko.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات jekko.
...
5. يمكنك أيضا تخصيصها عبر إعدادات jekko. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات jekko. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع jekko. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `jekko.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Jekko بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `jekko auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات jekko وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات jekko.
