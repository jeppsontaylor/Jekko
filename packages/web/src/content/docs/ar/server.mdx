---
title: الخادم
description: تفاعل مع خادم jekko عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `jekko serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل jekko استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `jekko.local` |
...
عيّن `JEKKO_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `jekko`، أو عيّن `JEKKO_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `jekko serve` و `jekko web`.
...
عند تشغيل `jekko` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم jekko للتفاعل مع jekko برمجيا.
:::

تتيح هذه البنية لـ jekko دعم عدة عملاء وتمكّنك من التفاعل مع jekko برمجيا.

يمكنك تشغيل `jekko serve` لبدء خادم مستقل. إذا كانت واجهة jekko في terminal (TUI)
قيد التشغيل، فسيبدأ `jekko serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Jekko لـ [IDE](/docs/ide).
...
يعرض خادم jekko واجهات API التالية.
