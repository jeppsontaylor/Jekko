---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع Jekko.
---

تتيح لك الإضافات توسيع Jekko عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ Jekko.
...
- `.jekko/plugins/` - إضافات على مستوى المشروع
- `~/.config/jekko/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/jekko/node_modules/`.
...
1. إعدادات عامة (`~/.config/jekko/jekko.json`)
2. إعدادات المشروع (`jekko.json`)
3. دليل الإضافات العام (`~/.config/jekko/plugins/`)
4. دليل إضافات المشروع (`.jekko/plugins/`)
...
يشغّل Jekko الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ jekko للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع jekko.
...
:::note
إذا كنت تستخدم تطبيق Jekko لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى jekko:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ jekko استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ jekko إلى جانب الأدوات المضمنة.
...
المستويات: `debug`, `info`, `warn`, `error`. راجع [توثيق SDK](https://jekko.ai/docs/sdk) للتفاصيل.
