...
jekko ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak jekko kendi `jekko` temasını kullanır.
...
jekko birden fazla yerleşik temayla gelir.
...
- jekko'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
jekko, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/jekko/themes/*.json` veya `$XDG_CONFIG_HOME/jekko/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.jekko/themes/*.json`
4. **Geçerli çalışma dizini** - `./.jekko/themes/*.json`
