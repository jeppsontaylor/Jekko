...
_Model Bağlam Protokolü_ veya MCP kullanarak jekko'a harici araçlar ekleyebilirsiniz. jekko hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [jekko Config](https://jekko.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/jekko` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
jekko, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde jekko şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa jekko, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `jekko mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından jekko, belirteçleri `~/.local/share/jekko/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra jekko'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi jekko yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve jekko'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
