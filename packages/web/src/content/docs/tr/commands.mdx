...
`.jekko/commands/test.md` oluştur:
...
jekko işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

jekko [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/jekko/commands/`
- Proje başına: `.jekko/commands/`
...
jekko, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
