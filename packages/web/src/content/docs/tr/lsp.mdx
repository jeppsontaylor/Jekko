...
description: Jekko, LSP sunucularınızla bütünleşir.
---

Jekko, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

Jekko, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`JEKKO_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP sunucularını, jekko yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
