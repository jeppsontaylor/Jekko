---
title: TUI
description: Jekko terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Jekko, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

Jekko'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
Jekko TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
Jekko'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
Jekko'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `jekko.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `JEKKO_TUI_CONFIG` kullanın.
