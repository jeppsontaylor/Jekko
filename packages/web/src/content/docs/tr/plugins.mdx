---
title: Eklentiler
description: Jekko'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Jekko'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Jekko'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.jekko/plugins/` - Proje düzeyinde eklentiler
- `~/.config/jekko/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/jekko/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/jekko/jekko.json`)
2. Project config (`jekko.json`)
3. Global eklenti dizini (`~/.config/jekko/plugins/`)
4. Proje eklenti dizini (`.jekko/plugins/`)
...
Jekko bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Jekko SDK istemcisi.
...
Jekko'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Jekko masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Jekko'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Jekko'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Jekko'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Jekko'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://jekko.ai/docs/sdk) bakın.
