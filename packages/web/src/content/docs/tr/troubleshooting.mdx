...
jekko ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\jekko\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `jekko --log-level DEBUG`.

---

## Depolama

jekko, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/jekko/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\jekko` yapistirin
...
jekko Desktop arka planda yerel bir jekko sunucusu (`jekko-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `jekko` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/jekko/jekko.jsonc` (veya `~/.config/jekko/jekko.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/jekko/jekko.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\jekko\jekko.jsonc` yapistirin
...
jekko diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/jekko/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\jekko\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), jekko'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. jekko Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/jekko`
- **Linux**: `~/.cache/jekko` dizinini silin (veya `rm -rf ~/.cache/jekko` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\jekko` yapistirin

3. jekko Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

jekko Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`jekko.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `JEKKO_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `JEKKO_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta jekko Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, jekko ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

jekko Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda jekko icin bildirimler etkinse
...
1. jekko Desktop'u kapatin
2. Su dosyalari bulun ve silin (jekko Desktop uygulama veri dizininde yer alirlar):

- `jekko.settings.dat` (desktop varsayilan sunucu URL'si)
- `jekko.global.dat` ve `jekko.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
jekko ile ilgili bir sorun yasiyorsaniz:
...
3. `jekko upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `jekko models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\jekko`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. jekko, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\jekko`

2. En guncel provider paketlerini yeniden kurmak icin jekko'u yeniden baslatin

Bu, jekko'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
jekko, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
