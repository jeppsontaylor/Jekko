...
Jekko Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
Jekko Go şu anda beta olarak mevcut.
:::

Go, Jekko'daki diğer sağlayıcılar gibi çalışır. Jekko Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Jekko'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Jekko Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Jekko Go, Jekko'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Jekko Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Jekko Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Jekko Go'ya abone olabilir.
:::
...
Jekko Go aşağıdaki limitleri içerir:
...
Jekko yapılandırmanızdaki [model id](/docs/config/#models) formatı `jekko-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `jekko-go/kimi-k2.6` kullanmalısınız.
...
Jekko Go'yu şu amaçlarla oluşturduk:
...
4. Jekko ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
