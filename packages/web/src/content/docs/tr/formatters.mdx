...
description: jekko dile özgü biçimlendiriciler kullanır.
---

jekko, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

jekko, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, jekko bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

jekko bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri jekko yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.
