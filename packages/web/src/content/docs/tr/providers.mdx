---
title: Sağlayıcılar
description: jekko'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

jekko, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı jekko yapılandırmanızda yapılandırın.
...
`~/.local/share/jekko/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları jekko'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Jekko Zen, jekko ekibi tarafından sağlanan modellerin bir listesidir.
jekko ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Jekko Zen ile başlamanızı öneririz.
:::
...
jekko'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Jekko Go

Jekko Go, jekko ile iyi çalıştığı test edilmiş ve doğrulanmış, jekko ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Jekko Go`'yu seçin ve [jekko.ai/auth](https://jekko.ai/zen) adresine gidin.
...
Amazon Bedrock'u jekko ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   jekko'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `jekko.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi jekko'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
jekko'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   jekko'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
to store token in jekko auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
jekko, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. jekko'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`jekko.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [jekko-gitlab-auth](https://www.npmjs.com/package/jekko-gitlab-auth) ana sayfasında.

##### Yapılandırma

`jekko.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi jekko'la kullanmak için:
...
Google Vertex AI'yi jekko ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları jekko çalıştırılırken ayarlayın.
...
Helicone'un jekko aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her jekko görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [jekko-helicone-session](https://github.com/H2Shami/jekko-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her jekko görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
jekko'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla jekko'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla jekko'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini jekko için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/jekko)'a bakın.
:::
...
Ollama Cloud'u jekko ile kullanmak için:
...
3. jekko'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: jekko'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Jekko Zen, jekko ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Jekko Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Jekko Zen**'i arayın.

   ```txt
   /connect
   ```

3. jekko API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   jekko yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca jekko yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri jekko yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı jekko ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `jekko.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları jekko'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `jekko auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için jekko yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, jekko yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
