...
Ajan becerileri, jekko'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
jekko su konumlari tarar:

- Proje konfigurasyonu: `.jekko/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/jekko/skills/<name>/SKILL.md`
...
Proje yerel yollarinda jekko, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.jekko/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/jekko/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.jekko/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
jekko, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`jekko.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`jekko.json` icinde):
