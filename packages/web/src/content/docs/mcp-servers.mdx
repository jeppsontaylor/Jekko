...
You can add external tools to Jekko using the _Model Context Protocol_, or MCP. Jekko supports both local and remote servers.
...
You can define MCP servers in your [Jekko Config](https://jekko.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/jekko` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Jekko automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Jekko will:
...
If the server requires authentication, Jekko will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `jekko mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Jekko will store the tokens securely in `~/.local/share/jekko/mcp-auth.json`.
...
Your MCPs are available as tools in Jekko, alongside built-in tools. So you can manage them through the Jekko config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Jekko to your Sentry account.
