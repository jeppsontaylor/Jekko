...
Il existe deux types d'agents dans Jekko ; agents primaires et sous-agents.
...
Jekko est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Jekko est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

Jekko est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `jekko.json` :
...
- Global : `~/.config/jekko/agents/`
- Par projet : `.jekko/agents/`

```markdown title="~/.config/jekko/agents/review.md"
---
...
Si aucune température n'est spécifiée, Jekko utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Jekko et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Jekko utilise le format `provider/model-id`. Par exemple, si vous utilisez [Jekko Zen](/docs/zen), vous utiliserez `jekko/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="jekko.json" {3-6,9-12}
{
  "$schema": "https://jekko.ai/config.json",
  "tools": {
    "write": true,
...
```json title="jekko.json" {3-5,8-10}
{
  "$schema": "https://jekko.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `jekko models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/anomalyco/jekko).
:::
...
```markdown title="~/.config/jekko/agents/docs-writer.md"
---
...
```markdown title="~/.config/jekko/agents/security-auditor.md"
---
