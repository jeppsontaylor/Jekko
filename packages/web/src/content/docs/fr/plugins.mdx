---
title: Plugins
description: Écrivez vos propres plugins pour étendre Jekko.
---

Les plugins vous permettent d'étendre Jekko en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Jekko.
...
- `.jekko/plugins/` - Plugins au niveau du projet
- `~/.config/jekko/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/jekko/node_modules/`.
...
1. Configuration globale (`~/.config/jekko/jekko.json`)
2. Configuration du projet (`jekko.json`)
3. Répertoire global des plugins (`~/.config/jekko/plugins/`)
4. Répertoire des plugins du projet (`.jekko/plugins/`)
...
Jekko exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK jekko pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre jekko.
...
:::note
Si vous utilisez l'application de bureau Jekko, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez jekko de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à jekko :
...
L'assistant `tool` crée un outil personnalisé que jekko peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour jekko aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://jekko.ai/docs/sdk) pour plus de détails.
