...
Avec Jekko, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Jekko utilise notre propre thème `jekko`.
...
Jekko est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Jekko corresponde à l'apparence de leur terminal
...
Jekko prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/jekko/themes/*.json` ou `$XDG_CONFIG_HOME/jekko/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.jekko/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.jekko/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/jekko/themes
vim ~/.config/jekko/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .jekko/themes
vim .jekko/themes/my-theme.json
```
