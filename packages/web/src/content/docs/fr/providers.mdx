---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Jekko.
---

import config from "../../../../config.mjs"
export const console = config.console

Jekko utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Jekko.
...
en `~/.local/share/jekko/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Jekko
configuration.
...
Jekko Zen est une liste de modèles fournis par l'équipe Jekko qui ont été
testé et vérifié pour fonctionner correctement avec Jekko. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Jekko Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez jekko et dirigez-vous vers [jekko.ai/auth](https://jekko.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Jekko et son utilisation est totalement facultative.

---

## Jekko Go

Jekko Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Jekko qui ont été
testé et vérifié pour fonctionner correctement avec Jekko.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `Jekko Go` et rendez-vous sur [jekko.ai/auth](https://jekko.ai/zen).
...
Pour utiliser Amazon Bedrock avec Jekko :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de jekko :

```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY jekko

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile jekko

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX jekko
```
...
Pour une configuration spécifique au projet ou persistante, utilisez `jekko.json` :

```json title="jekko.json"
{
  "$schema": "https://jekko.ai/config.json",
  "provider": {
    "amazon-bedrock": {
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Jekko n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer jekko pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).

```json title="jekko.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://jekko.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que jekko fonctionne correctement.
   :::
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX jekko
   ```
...
Vous pouvez également ajouter des modèles via votre configuration jekko.
...
pour stocker le jeton dans le stockage d'authentification jekko.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Jekko utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Jekko
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `jekko.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [jekko-gitlab-auth](https://www.npmjs.com/package/jekko-gitlab-auth).

##### Configuration

Personnalisez via `jekko.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec jekko :
...
Pour utiliser Google Vertex AI avec Jekko :
...
Définissez-les lors de l'exécution de jekko.

```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id jekko
```
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via jekko, vous pouvez toujours le configurer vous-même.
...
```jsonc title="~/.config/jekko/jekko.jsonc"
{
  "$schema": "https://jekko.ai/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [jekko-helicone-session](https://github.com/H2Shami/jekko-helicone-session) pour enregistrer automatiquement chaque conversation Jekko en tant que session dans Helicone.

```bash
npm install -g jekko-helicone-session
```
...
```json title="jekko.json"
{
  "plugin": ["jekko-helicone-session"]
}
```

Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Jekko répertoriée comme une session distincte.
...
Vous pouvez configurer jekko pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)

```json title="jekko.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://jekko.ai/config.json",
  "provider": {
    "llama.cpp": {
...
Vous pouvez configurer jekko pour utiliser des modèles locaux via LM Studio.

```json title="jekko.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://jekko.ai/config.json",
  "provider": {
    "lmstudio": {
...
Vous pouvez configurer jekko pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Jekko. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/jekko) pour plus de détails.
:::

```json title="jekko.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://jekko.ai/config.json",
  "provider": {
    "ollama": {
...
Pour utiliser Ollama Cloud avec Jekko :
...
3. Copiez la clé API à utiliser dans Jekko.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Jekko, vous devez extraire les informations du modèle localement :
...
Jekko Zen est une liste de modèles testés et vérifiés fournie par l'équipe Jekko. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Jekko Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Jekko Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Jekko API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration jekko.
...
5. Vous pouvez également les personnaliser via votre configuration jekko. Voici un exemple de spécification d'un fournisseur
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' jekko
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group jekko
   ```
...
Vous pouvez également personnaliser les modèles via votre configuration jekko. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec jekko. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
3. Entrez votre clé API pour le fournisseur.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in jekko.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Créez ou mettez à jour votre fichier `jekko.json` dans le répertoire de votre projet :

   ```json title="jekko.json" ""myprovider"" {5-15}
   {
     "$schema": "https://jekko.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="jekko.json" {9,11,17-20}
{
  "$schema": "https://jekko.ai/config.json",
  "provider": {
    "myprovider": {
...
Les champs `limit` permettent à Jekko de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `jekko auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration jekko et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration jekko.
