---
title: Serveur
description: Interagissez avec le serveur jekko via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `jekko serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client jekko peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `jekko.local` |
...
Définissez `JEKKO_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `jekko`, ou définissez `JEKKO_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `jekko serve` et à `jekko web`.
...
Lorsque vous exécutez `jekko`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur jekko pour interagir avec jekko par programmation.
:::

Cette architecture permet à jekko de prendre en charge plusieurs clients et vous permet d'interagir avec jekko par programme.

Vous pouvez exécuter `jekko serve` pour démarrer un serveur autonome. Si vous avez le
jekko TUI en cours d'exécution, `jekko serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Jekko [IDE](/docs/ide).
...
Le serveur jekko expose les API suivantes.
