...
Les outils permettent au LLM d'effectuer des actions dans votre base de code. Jekko est livré avec un ensemble d'outils intégrés, mais vous pouvez l'étendre avec [outils personnalisés](/docs/custom-tools) ou [serveurs MCP](/docs/mcp-servers).
...
Voici tous les outils intégrés disponibles dans Jekko.
...
:::note
Cet outil n'est disponible que lorsque `JEKKO_EXPERIMENTAL_LSP_TOOL=true` (ou `JEKKO_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur Jekko ou lorsque la variable d'environnement `JEKKO_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).

À activer lors du lancement de Jekko :
