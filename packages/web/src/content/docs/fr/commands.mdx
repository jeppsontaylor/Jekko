...
Vous pouvez ajouter des commandes personnalisées via la configuration Jekko ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre Jekko [config](/docs/config) :
...
jekko comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
