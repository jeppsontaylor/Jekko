---
title: Providers
description: Using any LLM provider in Jekko.
---

import config from "../../../config.mjs"
export const console = config.console

Jekko uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Jekko config.
...
You can customize the providers through the `provider` section in your Jekko
config.
...
Jekko Zen is a list of models provided by the Jekko team that have been
tested and verified to work well with Jekko. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Jekko Zen.
:::

1. Run the `/connect` command in the TUI, select `Jekko Zen`, and head to [jekko.ai/auth](https://jekko.ai/zen).
...
It works like any other provider in Jekko and is completely optional to use.

---

## Jekko Go

Jekko Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Jekko team that have been
tested and verified to work well with Jekko.

1. Run the `/connect` command in the TUI, select `Jekko Go`, and head to [jekko.ai/auth](https://jekko.ai/zen).
...
To use Amazon Bedrock with Jekko:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running jekko:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `jekko.json`:
...
Jekko. Anthropic explicitly prohibits this.

Previous versions of Jekko came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Jekko with zero setup:
...
You can configure jekko to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for jekko to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your jekko config.
...
GitLab Duo support in Jekko is experimental. Features, configuration, and
behavior may change in future releases.
:::

Jekko integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
Jekko uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Jekko
to only use your own GitLab-hosted instance, add the following to your
`jekko.json` file. It is also recommended to disable session sharing.
...
More documentation on [jekko-gitlab-auth](https://www.npmjs.com/package/jekko-gitlab-auth) homepage.
...
When a `duo-workflow-*` model is selected, Jekko will:
...
4. Route tool execution requests through Jekko's permission-gated tool system
...
To use your GitHub Copilot subscription with jekko:
...
To use Google Vertex AI with Jekko:
...
In the event you see a feature or model from Helicone that isn't configured automatically through jekko, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [jekko-helicone-session](https://github.com/H2Shami/jekko-helicone-session) plugin to automatically log each Jekko conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Jekko conversation listed as a separate session.
...
You can configure jekko to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure jekko to use local models through LM Studio.
...
You can configure jekko to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Jekko. See the [Ollama integration docs](https://docs.ollama.com/integrations/jekko) for details.
:::
...
To use Ollama Cloud with Jekko:
...
3. Copy the API key for use in Jekko.
...
6. **Important**: Before using cloud models in Jekko, you must pull the model information locally:
...
Jekko Zen is a list of tested and verified models provided by the Jekko team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Jekko Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Jekko Zen**.

   ```txt
   /connect
   ```

3. Enter your Jekko API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your jekko config.
...
5. You can also customize them through your jekko config. Here's an example of specifying a provider
...
You can also customize models through your jekko config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with jekko. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `jekko.json` file in your project directory:
...
The `limit` fields allow Jekko to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `jekko auth list` to see if the credentials
...
2. For custom providers, check the jekko config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your jekko config.
