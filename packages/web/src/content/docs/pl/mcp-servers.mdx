...
Możesz dodać dodatkowe informacje do jekko za pomocą _Model Context Protocol_, w skrócie MCP. jekko obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [jekko Config](https://jekko.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/jekko`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
jekko automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, jekko:
...
Jeśli serwer wymaga uwierzytelnienia, jekko poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `jekko mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji jekko bezpieczne przechowa tokeny w `~/.local/share/jekko/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w jekko, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie jekko, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć jekko z kontem Sentry.
