...
Umiejętności agenta pozwalają jekko odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
jekko przeszukuje te lokalizacje:

- Project config: `.jekko/skills/<name>/SKILL.md`
- Global config: `~/.config/jekko/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu jekko przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.jekko/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/jekko/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.jekko/skills/git-release/SKILL.md` w ten sposób:
...
jekko wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `jekko.json`:
...
**Dla agentów wbudowanych** (w `jekko.json`):
