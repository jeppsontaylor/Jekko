...
Dzięki jekko możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie jekko używa naszego własnego motywu `jekko`.
...
jekko ma kilka wbudowanych motywów.
...
- Chcą, aby Jekko pasował do wyglądu ich terminala
...
jekko obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/jekko/themes/*.json` lub `$XDG_CONFIG_HOME/jekko/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.jekko/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.jekko/themes/*.json`
