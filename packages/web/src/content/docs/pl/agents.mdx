...
W Jekko obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Jekko zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Jekko ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

Jekko ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `jekko.json`:
...
- Globalnie: `~/.config/jekko/agents/`
- W projekcie: `.jekko/agents/`
...
Jeśli nie zostanie skonfigurowana, Jekko używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Jekko, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Jekko w `provider/model-id`. Na przykład, [Jekko Zen](/docs/zen), użyj `jekko/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `jekko models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/anomalyco/jekko).
:::
