---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć jekko.
---

Wtyczki umożliwiające wyłączenie jekko poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania jekko.
...
- `.jekko/plugins/` - Wtyczki na poziomie projektu
- `~/.config/jekko/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/jekko/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/jekko/jekko.json`)
2. Konfiguracja projektu (`jekko.json`)
3. Globalny katalog wtyczek (`~/.config/jekko/plugins/`)
4. Katalog wtyczek projektu (`.jekko/plugins/`)
...
jekko uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z jekko do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do jekko.
...
:::note
Jeśli korzystasz z aplikacji komputerowej jekko, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj jekko czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do jekko:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod jekko. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://jekko.ai/docs/sdk).
