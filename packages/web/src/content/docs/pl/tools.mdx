...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Jekko posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Jekko.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `JEKKO_EXPERIMENTAL_LSP_TOOL=true` (lub `JEKKO_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Jekko lub gdy zmienna środowiskowa `JEKKO_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Jekko:
