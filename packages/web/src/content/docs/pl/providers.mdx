---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w jekko.
---

import config from "../../../../config.mjs"
export const console = config.console

jekko używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji jekko.
...
w `~/.local/share/jekko/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim jekko
config.
...
Jekko Zen to lista modeli dostarczonych przez zespół jekko, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z jekko. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Jekko Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Jekko Zen` i przejdź do [jekko.ai/auth](https://jekko.ai/zen).
...
Działa jak każdy inny dostawca w jekko i jest całkowicie opcjonalny w użyciu.

---

## Jekko Go

Jekko Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół jekko, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z jekko.

1. Uruchom polecenie `/connect` w TUI, wybierz `Jekko Go` i przejdź do [jekko.ai/auth](https://jekko.ai/zen).
...
Aby używać Amazon Bedrock z jekko:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania jekko:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `jekko.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w jekko nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować jekko do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod jekko działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji jekko.
...
jekko używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować jekko
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`jekko.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [jekko-gitlab-auth](https://www.npmjs.com/package/jekko-gitlab-auth).

##### Konfiguracja

Customize through `jekko.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem jekko:
...
Aby używać Google Vertex AI z jekko:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania jekko.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą jekko, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [jekko-helicone-session](https://github.com/H2Shami/jekko-helicone-session), aby automatycznie rejestrować każdą konwersację jekko jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja jekko będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować jekko tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować jekko tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować jekko tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem jekko. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/jekko).
:::
...
Aby korzystać z Ollama Cloud z jekko:
...
3. Skopiuj klucz API do użycia w jekko.
...
6. **Ważne**: Przed użyciem modeli chmurowych w jekko musisz pobrać informacje o modelu lokalnie:
...
Jekko Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół jekko. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Jekko Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Jekko Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API jekko.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji jekko.
...
5. Możesz je także dostosować za pomocą konfiguracji jekko. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z jekko:
...
Możesz także dostosować modele za pomocą konfiguracji jekko. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z jekko. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `jekko.json` w katalogu projektu:
...
Pola `limit` pozwalają jekko zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `jekko auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację jekko i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji jekko.
