...
Aby debugować problemy z jekko, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\jekko\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `jekko --log-level DEBUG`.

---

## Przechowywanie danych

jekko przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/jekko/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\jekko`
...
jekko Desktop uruchamia w tle lokalny serwer jekko (sidecar `jekko-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Jekko` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/jekko/jekko.jsonc` (lub `~/.config/jekko/jekko.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/jekko/jekko.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\jekko\jekko.jsonc`
...
jekko może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/jekko/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\jekko\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby jekko mógł ją odbudować.

1. Całkowicie zamknij jekko Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/jekko`
- **Linux**: usuń `~/.cache/jekko` (lub uruchom `rm -rf ~/.cache/jekko`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\jekko`

3. Uruchom ponownie jekko Desktop.

---

### Napraw problemy z połączeniem z serwerem

jekko Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `jekko.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `JEKKO_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `JEKKO_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows jekko Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami jekko.

---

### Brak powiadomień

jekko Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla jekko w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie jekko Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji jekko Desktop):

- `jekko.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `jekko.global.dat` i `jekko.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `jekko upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `jekko models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\jekko`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. jekko dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\jekko`

2. Uruchom ponownie kod jekko, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to jekko do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
jekko wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
