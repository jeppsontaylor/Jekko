...
你可以通过 _Model Context Protocol_（MCP）为 Jekko 添加外部工具。Jekko 同时支持本地和远程服务器。
...
你可以在 [Jekko 配置](https://jekko.ai/docs/config/)的 `mcp` 字段下定义 MCP 服务器。为每个 MCP 指定一个唯一的名称，在提示词中可以通过该名称来引用对应的 MCP。
...
组织可以通过其 `.well-known/jekko` 端点提供默认的 MCP 服务器。这些服务器可能默认处于禁用状态，允许用户按需启用。
...
Jekko 会自动处理远程 MCP 服务器的 OAuth 身份验证。当服务器需要身份验证时，Jekko 将：
...
如果服务器需要身份验证，Jekko 会在你首次使用时提示你进行认证。你也可以使用 `jekko mcp auth <server-name>` [手动触发认证流程](#authenticating)。
...
`mcp auth` 命令会打开浏览器进行授权。授权完成后，Jekko 会将 Token 安全地存储在 `~/.local/share/jekko/mcp-auth.json` 中。
...
你的 MCP 在 Jekko 中作为工具使用，与内置工具并列。因此，你可以像管理其他工具一样，通过 Jekko 配置来管理它们。
...
这会打开浏览器窗口完成 OAuth 流程，将 Jekko 连接到你的 Sentry 账户。
