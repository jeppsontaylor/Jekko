...
Le skill degli agenti permettono a Jekko di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Jekko cerca in queste posizioni:

- Config di progetto: `.jekko/skills/<name>/SKILL.md`
- Config globale: `~/.config/jekko/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Jekko risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.jekko/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/jekko/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.jekko/skills/git-release/SKILL.md` cosi':
...
Jekko elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `jekko.json`:
...
**Per agenti integrati** (in `jekko.json`):
