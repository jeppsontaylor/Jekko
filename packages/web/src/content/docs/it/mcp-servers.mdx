...
Puoi aggiungere strumenti esterni a Jekko usando il _Model Context Protocol_, o MCP. Jekko supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Jekko Config](https://jekko.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/jekko`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Jekko gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Jekko:
...
Se il server richiede autenticazione, Jekko ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `jekko mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Jekko memorizzera' i token in modo sicuro in `~/.local/share/jekko/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Jekko insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Jekko come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Jekko al tuo account Sentry.
