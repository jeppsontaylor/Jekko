---
title: Plugin
description: Scrivi plugin per estendere Jekko.
---

I plugin ti permettono di estendere Jekko agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Jekko.
...
- `.jekko/plugins/` - plugin a livello progetto
- `~/.config/jekko/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/jekko/node_modules/`.
...
1. Config globale (`~/.config/jekko/jekko.json`)
2. Config di progetto (`jekko.json`)
3. Directory plugin globale (`~/.config/jekko/plugins/`)
4. Directory plugin di progetto (`.jekko/plugins/`)
...
Jekko esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di jekko per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere jekko.
...
:::note
Se usi l'app desktop di Jekko, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a jekko di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a jekko:
...
L'helper `tool` crea uno strumento personalizzato che jekko può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in jekko insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://jekko.ai/docs/sdk) per i dettagli.
