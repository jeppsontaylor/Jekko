...
In Jekko esistono due tipi di agenti: agenti primari e subagenti.
...
Jekko include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Jekko include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

Jekko include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `jekko.json`:
...
- Globale: `~/.config/jekko/agents/`
- Per progetto: `.jekko/agents/`
...
Se non specifichi una temperature, Jekko usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Jekko sia per la config specifica del progetto.
...
L'ID modello nella configurazione Jekko usa il formato `provider/model-id`. Per esempio, se stai usando [Jekko Zen](/docs/zen), useresti `jekko/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `jekko models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/jekko).
:::
