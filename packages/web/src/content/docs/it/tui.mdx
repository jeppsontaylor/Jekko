---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Jekko.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Jekko fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Jekko avvia la TUI per la directory corrente.
...
Quando usi la TUI di Jekko, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Jekko. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Questo è separato da `jekko.json`, che configura il comportamento di server/runtime.
...
Usa `JEKKO_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
