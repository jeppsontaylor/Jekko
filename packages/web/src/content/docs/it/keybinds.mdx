...
Jekko ha una lista di scorciatoie che puoi personalizzare tramite `tui.json`.
...
Jekko usa un tasto `leader` per la maggior parte delle scorciatoie. Questo evita conflitti nel terminale.
...
Il campo di input del prompt nell'app desktop di Jekko supporta scorciatoie comuni in stile Readline/Emacs per modificare il testo. Sono integrate e al momento non configurabili tramite `jekko.json`.
