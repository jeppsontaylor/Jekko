---
title: Server LSP
description: Jekko si integra con i tuoi server LSP.
---

Jekko si integra con il tuo Language Server Protocol (LSP) per aiutare l'LLM a interagire con la tua codebase. Usa le diagnostiche per fornire feedback all'LLM.

---

## Integrati

Jekko include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `JEKKO_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Puoi personalizzare i server LSP tramite la sezione `lsp` nella configurazione di jekko.
