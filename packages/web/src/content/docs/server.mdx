---
title: Server
description: Interact with jekko server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `jekko serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an jekko client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `jekko.local` |
...
Set `JEKKO_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `jekko`, or set `JEKKO_SERVER_USERNAME` to override it. This applies to both `jekko serve` and `jekko web`.
...
When you run `jekko` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the jekko server to interact with jekko programmatically.
:::

This architecture lets jekko support multiple clients and allows you to interact with jekko programmatically.

You can run `jekko serve` to start a standalone server. If you have the
jekko TUI running, `jekko serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Jekko [IDE](/docs/ide) plugins.
...
The jekko server exposes the following APIs.
