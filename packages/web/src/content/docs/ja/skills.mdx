...
エージェントスキルにより、Jekko はリポジトリまたはホームディレクトリから再利用可能な命令を検出できます。
...
- プロジェクト設定: `.jekko/skills/<name>/SKILL.md`
- グローバル設定: `~/.config/jekko/skills/<name>/SKILL.md`
...
プロジェクトのローカルパスの場合、Jekko は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
途中で、一致する `skills/*/SKILL.md` を `.jekko/` に読み込み、一致する `.claude/skills/*/SKILL.md` または `.agents/skills/*/SKILL.md` を読み込みます。

グローバル定義は、`~/.config/jekko/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md`、および `~/.agents/skills/*/SKILL.md` からもロードされます。
...
次のように `.jekko/skills/git-release/SKILL.md` を作成します。
...
Jekko では、`skill` ツールの説明に利用可能なスキルがリストされています。
...
`jekko.json` のパターンベースの権限を使用して、エージェントがアクセスできるスキルを制御します。
