---
title: プラグイン
description: Jekko を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Jekko を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Jekko のデフォルトの動作を変更したりできます。
...
- `.jekko/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/jekko/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/jekko/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/jekko/jekko.json`)
2. プロジェクト設定 (`jekko.json`)
3. グローバルプラグインディレクトリ (`~/.config/jekko/plugins/`)
4. プロジェクトプラグインディレクトリ (`.jekko/plugins/`)
...
Jekko は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Jekko SDK クライアント。
...
ここでは、Jekko を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://jekko.ai/docs/sdk) を参照してください。
...
プラグインは Jekko にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Jekko が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Jekko で使用できます。
