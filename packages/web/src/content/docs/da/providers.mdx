---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Jekko.
---

import config from "../../../../config.mjs"
export const console = config.console

Jekko bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Jekko-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/jekko/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Jekko-konfigurationen.
...
Jekko Zen er en liste over modeller leveret af Jekko-teamet, som er blevet testet og verificeret til at fungere godt med Jekko. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Jekko Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg jekko og gå til [jekko.ai/auth](https://jekko.ai/auth).
...
Det fungerer som alle andre udbydere i Jekko og er helt valgfrit at bruge.

---

## Jekko Go

Jekko Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Jekko-teamet, som er testet og verificeret til at fungere godt med Jekko.

1. Kør kommandoen `/connect` i TUI, vælg `Jekko Go`, og gå til [jekko.ai/auth](https://jekko.ai/zen).
...
Sådan bruger du Amazon Bedrock med Jekko:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører jekko:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `jekko.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Jekko understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere jekko til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at jekko skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem jekko-konfigurationen.
...
gemme tokenet i jekko auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Jekko bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Jekko
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`jekko.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [jekko-gitlab-auth](https://www.npmjs.com/package/jekko-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `jekko.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Jekko:
...
Sådan bruger du Google Vertex AI med Jekko:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem jekko, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [jekko-helicone-session](https://github.com/H2Shami/jekko-helicone-session) plugin for automatisk at logge hver Jekko-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Jekko-samtale opført som en separat session.
...
Du kan konfigurere jekko til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere jekko til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere jekko til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Jekko. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/jekko) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Jekko:
...
3. Kopier API-nøglen til brug i Jekko.
...
6. **Vigtigt**: Før du bruger skymodeller i Jekko, skal du hente modelinformationen lokalt:
...
Jekko Zen er en liste over testede og verificerede modeller leveret af Jekko-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Jekko Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Jekko Zen**.

   ```txt
   /connect
   ```

3. Indtast Jekko API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem jekko-konfigurationen.
...
5. Du kan også tilpasse dem gennem jekko-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Jekko:
...
Du kan også tilpasse modeller gennem jekko-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med jekko. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `jekko.json`-filen i projektkataloget:
...
`limit`-felterne lader Jekko forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `jekko auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek jekko-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i jekko-konfigurationen.
