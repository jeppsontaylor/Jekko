...
Med Jekko kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Jekko vårt eget `jekko`-tema.
...
Jekko støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/jekko/themes/*.json` eller `$XDG_CONFIG_HOME/jekko/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.jekko/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.jekko/themes/*.json`
