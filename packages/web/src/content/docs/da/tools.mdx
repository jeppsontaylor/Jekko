...
Her er alle de innebygde verktøyene som er tilgængelige i Jekko.
...
:::note
Dette verktøyet er bare tilgængelig når `JEKKO_EXPERIMENTAL_LSP_TOOL=true` (eller `JEKKO_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Jekko-leverandøren eller når miljøvariabelen `JEKKO_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
