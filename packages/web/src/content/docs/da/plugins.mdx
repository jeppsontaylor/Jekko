---
title: Plugins
description: Skriv dine egne plugins for at udvide Jekko.
---

Plugins giver dig mulighed for at udvide Jekko ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Jekkos standardadfærd.
...
- `.jekko/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/jekko/node_modules/`.
...
1. Global konfiguration (`~/.config/jekko/jekko.json`)
2. Projektkonfiguration (`jekko.json`)
3. Global plugin-mappe (`~/.config/jekko/plugins/`)
4. Projekt plugin bibliotek (`.jekko/plugins/`)
...
Jekko kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En jekko SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide jekko.
...
:::note
Hvis du bruger Jekko desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå jekko i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til jekko:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som jekko kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for jekko sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://jekko.ai/docs/sdk) for detaljer.
