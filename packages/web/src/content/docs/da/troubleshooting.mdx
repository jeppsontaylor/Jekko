...
For at fejlfinde problemer med Jekko, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\jekko\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `jekko --log-level DEBUG`.

---

## Lagring

jekko gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/jekko/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\jekko`
...
Jekko Desktop kører en lokal Jekko-server (`jekko-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Jekko`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/jekko/jekko.jsonc` (eller `~/.config/jekko/jekko.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/jekko/jekko.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\jekko\jekko.jsonc`
...
Jekko kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/jekko/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\jekko\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Jekko kan genopbygge den.

1. Afslut Jekko Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/jekko`
- **Linux**: slet `~/.cache/jekko` (eller kør `rm -rf ~/.cache/jekko`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\jekko`

3. Start Jekko Desktop på ny.

---

### Løs problemer med serverforbindelse

Jekko Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `jekko.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `JEKKO_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `JEKKO_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Jekko Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Jekko.

---

### Meddelelser vises ikke

Jekko Desktop viser kun systemvarsler når:

- varsler er aktiveret for Jekko i dine OS-indstillinger, og
...
1. Afslut Jekko Desktop.
2. Find og slet disse filer (de findes i Jekko Desktop-appens datamappe):

- `jekko.settings.dat` (skrivebordsstandardserver URL)
- `jekko.global.dat` og `jekko.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Jekko:
...
For at finde ud af hvilke modeller du har adgang til, kør `jekko models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\jekko`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. jekko installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\jekko`

2. Start jekko på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge jekko til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
jekko vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
