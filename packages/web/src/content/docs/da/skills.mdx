...
Agentferdigheter lar Jekko oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Jekko søker etter disse stedene:

- Prosjektkonfigurasjon: `.jekko/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/jekko/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Jekko opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.jekko/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/jekko/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.jekko/skills/git-release/SKILL.md` slik:
...
Jekko viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `jekko.json`:
...
**For innebygde agenter** (i `jekko.json`):
