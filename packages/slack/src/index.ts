...
import { createOpencode, type ToolPart } from "@jekko-ai/sdk"
...
console.log("🚀 Starting jekko server...")
const jekko = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await jekko.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new jekko session...")
    const { client, server } = jekko
    ...
    console.log("✅ Created jekko session:", createResult.data.id)
  ...
  console.log("📝 Sending to jekko:", message.text)
