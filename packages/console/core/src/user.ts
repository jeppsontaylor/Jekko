  ...
  export const invite = fn(
    ...
    async ({ email, role, monthlyLimit }) => {
      ...
      try {
        ...
        const { InviteEmail } = await import("@jekko-ai/console-mail/InviteEmail.jsx")
        await AWS.sendEmail({
          to: email,
          subject: `You've been invited to join the ${emailInfo.workspaceName} workspace on Jekko`,
          body: render(
            // @ts-ignore
            InviteEmail({
              inviter: emailInfo.inviterEmail,
              assetsUrl: `https://jekko.ai/email`,
