...
brew install anomalyco/tap/jekko # macOS og Linux (anbefalt, alltid oppdatert)
brew install jekko              # macOS og Linux (offisiell brew-formel, oppdateres sjeldnere)
...
Jekko er også tilgjengelig som en desktop-app. Last ned direkte fra [releases-siden](https://github.com/anomalyco/jekko/releases) eller [jekko.ai/download](https://jekko.ai/download).
...
1. `$JEKKO_INSTALL_DIR` - Egendefinert installasjonsmappe
...
Jekko har to innebygde agents du kan bytte mellom med `Tab`-tasten.
...
Les mer om [agents](https://jekko.ai/docs/agents).

### Dokumentasjon

For mer info om hvordan du konfigurerer Jekko, [**se dokumentasjonen**](https://jekko.ai/docs).

### Bidra

Hvis du vil bidra til Jekko, les [contributing docs](./CONTRIBUTING.md) før du sender en pull request.

### Bygge på Jekko

Hvis du jobber med et prosjekt som er relatert til Jekko og bruker "jekko" som en del av navnet; for eksempel "jekko-dashboard" eller "jekko-mobile", legg inn en merknad i README som presiserer at det ikke er bygget av Jekko-teamet og ikke er tilknyttet oss på noen måte.
...
- 100% open source
- Ikke knyttet til en bestemt leverandør. Selv om vi anbefaler modellene vi tilbyr gjennom [Jekko Zen](https://jekko.ai/zen); kan Jekko brukes med Claude, OpenAI, Google eller til og med lokale modeller. Etter hvert som modellene utvikler seg vil gapene lukkes og prisene gå ned, så det er viktig å være provider-agnostic.
- LSP-støtte rett ut av boksen
- Fokus på TUI. Jekko er bygget av neovim-brukere og skaperne av [terminal.shop](https://terminal.shop); vi kommer til å presse grensene for hva som er mulig i terminalen.
- Klient/server-arkitektur. Dette kan for eksempel la Jekko kjøre på maskinen din, mens du styrer den eksternt fra en mobilapp. Det betyr at TUI-frontend'en bare er en av de mulige klientene.

---

**Bli med i fellesskapet** [Discord](https://discord.gg/jekko) | [X.com](https://x.com/jekko)
