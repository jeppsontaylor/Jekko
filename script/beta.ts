...
const model = "jekko/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/jekko")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with jekko...`)
  ...
  try {
    await $`jekko run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  jekko failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with jekko")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/jekko`.",
