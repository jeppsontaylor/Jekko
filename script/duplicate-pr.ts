...
import { createOpencode } from "@jekko-ai/sdk"
...
`)
  ...
  const jekko = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await jekko.client.session.create()
    const result = await jekko.client.session
      .prompt({
