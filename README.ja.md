...
# パッケージマネージャー
npm i -g jekko-ai@latest        # bun/pnpm/yarn でもOK
scoop install jekko             # Windows
choco install jekko             # Windows
brew install anomalyco/tap/jekko # macOS と Linux（推奨。常に最新）
brew install jekko              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S jekko            # Arch Linux (Stable)
paru -S jekko-bin               # Arch Linux (Latest from AUR)
mise use -g jekko               # どのOSでも
nix run nixpkgs#jekko           # または github:anomalyco/jekko で最新 dev ブランチ
```
...
1. `$JEKKO_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.jekko/bin` - デフォルトのフォールバック
...
Jekko には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://jekko.ai/docs/agents) の詳細はこちら。

### ドキュメント

Jekko の設定については [**ドキュメント**](https://jekko.ai/docs) を参照してください。

### コントリビュート

Jekko に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### Jekko の上に構築する

Jekko に関連するプロジェクトで、名前に "jekko"（例: "jekko-dashboard" や "jekko-mobile"）を含める場合は、そのプロジェクトが Jekko チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。
...
- 100% オープンソース
- 特定のプロバイダーに依存しません。[Jekko Zen](https://jekko.ai/zen) で提供しているモデルを推奨しますが、Jekko は Claude、OpenAI、Google、またはローカルモデルでも利用できます。モデルが進化すると差は縮まり価格も下がるため、provider-agnostic であることが重要です。
- そのまま使える LSP サポート
- TUI にフォーカス。Jekko は neovim ユーザーと [terminal.shop](https://terminal.shop) の制作者によって作られており、ターミナルで可能なことの限界を押し広げます。
- クライアント/サーバー構成。例えば Jekko をあなたのPCで動かし、モバイルアプリからリモート操作できます。TUI フロントエンドは複数あるクライアントの1つにすぎません。

---

**コミュニティに参加** [Discord](https://discord.gg/jekko) | [X.com](https://x.com/jekko)
